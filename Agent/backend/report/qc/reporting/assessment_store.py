...
def bots_in_slot(data_dir: Path, venue_type: str, symbol: str) -> Dict[str, str]:
    """uniqueCode -> nick_name for every bot the crawler filed under this
    (venue, symbol) directory, unchanged in behavior from before the unified
    layout -- used by `agent_server.py`'s `list_bots` tool.

    Deliberately NOT `_slot_index()` (universe/bot_selection.json): that is a
    LATER, separate decision (which bot fills which top/mid slot for scoring)
    that can legitimately disagree with where a bot was actually crawled.
    The fact this function answers -- "the crawler put this bot under
    <venue>/<asset>" -- used to be encoded by the old
    `<venue>/<asset>/bot/<folder>` directory nesting; the unified flat
    `data/trade/<bot_id>/` layout preserves it explicitly instead, in each
    bot's own `crawl_slot.json` (written once by the data-layout migration,
    see that script's own comment for why it cannot be re-derived).
    """
    wanted = (venue_type.upper(), symbol.upper())
    trade_root = Path(data_dir) / "trade"
    out: Dict[str, str] = {}
    if not trade_root.is_dir():
        return out
    for bot_dir in trade_root.iterdir():
        ...
        try:
            ...
            continue
        if (slot.get("venue"), slot.get("asset")) != wanted:
            continue
        code = bot_dir.name.removeprefix("bot_")
        nick_name = code
        overview_path = bot_dir / "overview.json"
        if overview_path.exists():
            try:
                overview = json.loads(overview_path.read_text(encoding="utf-8"))
                nick_name = overview.get("nickName") or code
            except (OSError, json.JSONDecodeError):
                pass
        out[code] = nick_name
    return out
...
def build_assessments(
    ...
    *,
    ...
    for row in report.rows:
        ...
        # Unified layout: one folder per bot_id, no venue/asset/name segments
        # (data/report/<bot_id>/) -- `venue`/`symbol` above are still computed
        # for the payload's own "<venue>/<symbol>" display label, just not for
        # the file path anymore.
        bot_dir = root / row.unique_code
        ...
        out.append((bot_dir / "latest.json", payload))
    return out
...
def persist(
    ...
    *,
    ...
    # nơi.
    codes_on_disk = {path.parent.name for path in root.glob("*/latest.json")}
...
def load_bot(
    ...
    `venue_type`/`symbol` are accepted for backward compatibility with every
    existing caller (see `Agent/backend/scripts/agent_server.py`) but are no
    longer part of the lookup path -- the unified layout is one folder per
    bot_id (`data/report/<unique_code>/`), not per venue/asset.
    """
    path = Path(data_dir) / ASSESSMENT_DIRNAME / unique_code / "latest.json"
    if not path.exists():
        return None
