...
class PairedBotReportService:
    ...
    def _locate(self, code: str) -> tuple[Optional[str], Optional[str]]:
        """The bot's own dominant market, not the slot's -- read from
        `crawl_slot.json` (unified layout: `data/trade/bot_<code>/` no
        longer nests under a venue/asset directory, see
        `Agent.backend.report.qc.reporting.assessment_store.bots_in_slot`'s
        docstring for why this is not re-derived from the slot itself).
        """
        bot_dir = self.data_dir / "trade" / f"bot_{code}"
