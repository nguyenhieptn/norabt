"""Proof that the live-source architecture (Agent/backend/external/sources/*) actually
...
Agent/backend/external/sources/bot_source.py's module docstring), so identical input
...
Both modes reuse Agent/backend/scripts/run_report.py's apply_live_source/
...
from Agent.backend.external.okx.client import OkxClient
from Agent.backend.report.qc.reporting.cohort import BotEvaluationRow, CohortAssessmentService
from Agent.backend.report.qc.reporting.render import _cell, _header, _stamp, _wrap
from Agent.backend.scripts.run_report import (
...
from Agent.backend.external.sources.market_source import MarketDataUnavailableError
...
class _StaticBotSource(BotDataSource):
    ...
    contract (see Agent/backend/external/sources/bot_source.py).
    """
...
def _locate_bot_folder(
    ...
    Mirrors PairedBotReportService._locate's own lookup: unified layout, so
    this is read from `crawl_slot.json`, not the (now flat) directory path.
    """
    bot_dir = data_dir / "trade" / f"bot_{unique_code}"
    slot_path = bot_dir / "crawl_slot.json"
    if not (bot_dir / "trade_list.json").exists() or not slot_path.exists():
        return None, None
    try:
        slot = json.loads(slot_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return None, None
    return slot.get("venue"), slot.get("asset")
