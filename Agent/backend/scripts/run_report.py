...
from Agent.backend.report.qc.reporting.analysis_store import persist as persist_analysis
from Agent.backend.report.qc.reporting.assessment_store import persist as persist_assessment
...
# --source live wiring.
#
# Every *Service class under Agent/backend/report/qc/reporting (DataReportService,
...
# their constructors in Agent/backend/report/qc/reporting/*.py) -- so the same
...
# Agent/backend/bot/mcp/analytics/strategy/profile.py's own module docstring --
...
def _horizon_scenarios_evidence(bot: Any) -> Optional[List[Dict[str, Any]]]:
    ...
    Agent/backend/bot/mcp/schemas/bot_result.py), reshaped to plain dicts for
...
def build_assessment_extras(
    ...
    *,
    ...
    from Agent.backend.llm import narrative as _narrative
