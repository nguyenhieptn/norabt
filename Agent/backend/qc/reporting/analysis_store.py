...
def persist(report: Any, data_dir: Path) -> List[str]:
    ...
    root = Path(data_dir) / ANALYSIS_DIRNAME
    written: List[str] = []
    ...
    for block in report.blocks:
        ...
        path = asset_dir / "market" / "market.json"
        _write(path, market)
...
def load_bot(
    ...
    Looked up by uniqueCode, not folder name: the name in the path is there for
    the reader and can change between crawls.
    """
    root = Path(data_dir) / ANALYSIS_DIRNAME / venue_type.lower() / symbol / "bot"
    if not root.is_dir():
        return None
    bot_dir = next(
