...
# depending on Agent/web/dist actually being built, so this suite never
...
def test_analyze_background_task_writes_snapshot_so_first_bot_report_click_is_fast(
    ...
    # 7 -> 8 svg, 12 -> 13 details: mục "Sụt giảm so với vốn" (tab Lệnh &
    # vị thế) thêm đúng một biểu đồ thanh + một khối phương pháp luận --
    # xem report_page.py::_render_drawdown_vs_capital.
    ...
    # 13 -> 14: khối trên đã được NỐI VÀO trang. Trước đây
    # `_render_score_basis` được viết đủ nhưng không nơi nào gọi, nên nó
    # không render và con số này đứng lại ở 13 dù chú thích ngay trên đã
    # mô tả nó -- test xanh vì chính cái nó mô tả đang mất tích.
...
# are Agent/test/test_narrative.py's own job; this section only checks the
...
def test_analyze_summary_surfaces_veto_decided_score() -> None:
    ...
    internal thresholds (which `Agent/test/test_quality_and_safety.py`
...
# Agent/test/test_narrative.py -- this section only checks the WIRING: the
...
class _PromptCapturingNarrativeBackend(narrative.NarrativeBackend):
    ...
    Agent/test/test_narrative.py's job)."""
...
def test_narrative_number_labels_never_embed_a_stray_digit() -> None:
    """Regression guard (see the matching test in Agent/test/test_narrative.py
...
def test_bot_report_snapshot_key_uses_agent_report_namespace_and_db1(
    ...
    unit level in Agent/test/test_snapshot.py's own
...
def test_bot_report_from_assessment_file_has_full_sections_and_charts(
    ...
    # Charts: dimension bars, win/loss count+profit pies, MC fan + drawdown.
...
def test_bot_report_from_assessment_file_with_chart_fields_matches_live_counts(
    ...
    assert "Probability of profit by horizon" in resp.text
