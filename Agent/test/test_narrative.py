...
`Agent/test/conftest.py` already strips every `NORABT_*` environment
...
# `Agent/test/test_report_page.py`'s `_strategy_result` uses, deliberately
...
# Agent/deploy/docker-compose.yml/README's Bước 12): the `claude` CLI has no
...
def test_home_passed_to_child_matches_configured_container_home() -> None:
    ...
    Agent/deploy/docker-compose.yml): the container's own `HOME` is set to
