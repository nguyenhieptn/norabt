...
def test_narrative_section_never_adds_svg_or_details(
    ...
    chart" regression test in Agent/test/test_web_app.py stay a fixed
...
def test_horizon_probability_chart_present_for_a_real_full_result(
    ...
    out = render_bot_report_html(full_result)
    assert "Probability of profit by horizon (chart)" in out
...
def test_horizon_probability_chart_hidden_when_scenarios_absent(
    ...
    out = render_bot_report_html(result)
    assert "Probability of profit by horizon (chart)" not in out
...
# --------------------------------------------------------------------------- #
# Shared design tokens (task's Việc 3) -- Agent/web/tokens.css is the single
...
_TOKENS_CSS_PATH = Path(config.BASE_DIR) / "web" / "tokens.css"
...
def test_tab_titles_renamed_and_verdict_wording_gone(
    ...
    out = render_bot_report_html(full_result)
    assert '<span class="tab-title">Analysis results</span>' in out
    assert '<span class="tab-title">Primary market</span>' in out
    assert '<span class="tab-title">Orders &amp; positions</span>' in out
