...
# and Agent/web -- the shipped product surface -- not Agent/test (whose own
...
def test_tokens_css_has_no_retired_verdict_label_in_a_comment_value_pair():
    css_text = (REPO_ROOT / "Agent" / "web" / "tokens.css").read_text(encoding="utf-8")
...
def test_real_bot_page_keeps_seven_svg_and_eleven_details(full_result):
    html = render_bot_report_html(full_result)
    # 7 -> 8: mục "Sụt giảm so với vốn" (tab Lệnh & vị thế) thêm ĐÚNG một
    # biểu đồ thanh "Năm lệnh lỗ nặng nhất" -- xem
    # `report_page.py::_render_drawdown_vs_capital`. Con số ở đây là một
    # bất biến CỐ Ý (bắt mọi thay đổi cấu trúc trang ngoài ý muốn), nên nó
    # chỉ được sửa cùng lúc với một mục mới có chủ đích như lần này.
