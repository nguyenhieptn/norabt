...
- **Mặc định vẫn TẮT**: cờ `X402_ENABLED`, mặc định `false`. Toàn bộ 520 test
  ...
  này -- đã xác nhận lại bằng `python3 -m pytest Agent/none/test -q` sau khi làm
...
`Agent/none/test/test_payments.py` duyệt qua từng nhánh này và khẳng định điều đó.
...
`Agent/none/test/test_agent_server.py`, gọi qua HTTP transport thật, không mock).
...
Test: `Agent/none/test/test_payments.py` (59 test) + phần x402 trong
`Agent/none/test/test_agent_server.py` (test `_require_payment`, và 2 test qua
