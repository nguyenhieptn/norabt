...
có test riêng (`Agent/test/test_research_splitting.py`).
...
python3 -m Agent.scripts.validate_out_of_sample --out-json /tmp/oos_results.json
```
...
- `Agent/scripts/validate_out_of_sample.py` — script chạy đầu-cuối.
- `Agent/test/test_research_splitting.py`,
  `Agent/test/test_research_outcomes.py`,
  `Agent/test/test_research_statistics.py` — test cho phần logic thuần.
