...
#
#   sudo bash Agent/nginx/install-nginx-vhost.sh
#
...
# Muốn mã hoá nốt chặng đó thì xem "Giai đoạn B" trong Agent/docker/README.md
...
TEMPLATE="$REPO_DIR/Agent/nginx/nginx-agent.conf.template"
...
if grep -q "zone=agent_analyze" "$NGINX_CONF"; then
    ...
    awk '
        !done && /^[[:space:]]*http[[:space:]]*\{/ {
            print
            print "\t# agent-web (agent.expsolution.io) -- xem Agent/nginx/nginx-agent.conf.template"
