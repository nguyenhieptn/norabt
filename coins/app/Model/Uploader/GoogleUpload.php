...
class GoogleUpload
{
    ...
    public function setDisk($disk)
    {
        ...
        if(!isset($diskConfig)) Reply::finish(false, 'Disk not found');
        if(!isset($diskConfig['driver'])) Reply::finish(false, 'Please define driver field');
        if(!isset($diskConfig['clientId'])) Reply::finish(false, 'Please define clientId field');
        if(!isset($diskConfig['clientSecret'])) Reply::finish(false, 'Please define clientSecret field');
        if(!isset($diskConfig['refreshToken'])) Reply::finish(false, 'Please define refreshToken field');
        if(!isset($diskConfig['folderId'])) Reply::finish(false, 'Please define folderId field');
        ...
        foreach ($dirs as $childir){
            ...
            if(!isset($this->listContents[$checkDir])){
                if(Storage::disk($this->disk)->makeDirectory($checkDirParent.'/'.$childir)){
                    ...
                    Reply::finish(false, 'Can not create folder:' . $checkDir);
                }
            }
        }
    }
        ...
        try{
            ...
            while (!$isValidName){
                if(!isset($this->listContents[$dir.'/'.$fileName])){
                    ...
                    $fileName = $path_parts['filename'].'_'.$rand.'.'.get($path_parts['extension'], '');
                    $rand ++;
                }
            }
    ...
    public function size($filePath){
        return $this->listContents[$filePath]['size'];
    }
