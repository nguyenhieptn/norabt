...
class LocalUpload
{
    ...
    public function setDisk($disk)
    {
        ...
        if(!isset($diskConfig['root'])) Reply::finish(false, 'Please define root field');
	 ...
	 try{
	       ...
	       if($condition != null){
                if(isset($condition->{'validation'})){
                    ...
                    if ($validator->fails()) {
                        return Reply::make(false, $validator->errors()->first());
                    }
                }
                
                if(isset($condition->{'regex'})){
                    if(!preg_match($condition->{'regex'}, $fileName)){
                        return Reply::make(false, 'File format is wrong');
                    }
                }
    ...
    public function read($filepath){
        try {
            ...
            return Reply::make(false, $e->getMessage());
        }
        ...
        try {
            ...
            foreach ($files as $file){
                $results[] = [
                    ...
                    'mime' => mime_content_type(Storage::disk($this->disk)->path($file)),
                    'modified' => filemtime(Storage::disk($this->disk)->path($file))
                ];
            }
