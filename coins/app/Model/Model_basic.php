...
class Model_basic extends Model
{
    ...
    public function filterToCondition($colID, $filterItem)
    {
        ...
        foreach ($fitlerData as $logic => $filter) {
            

            if(is_array($filter)){
               
                foreach($filter as $val){
                    $filterKey = [$colID, $logic, $val];
    ...
    public function getLastId(){
        return $this->query_builder->getConnection()->getPdo()->lastInsertId();
    }

    public function add($datas, $special = null, $trust = false, $options=[])
    {
        ...
        try {
            ...
            foreach ($datas as $key => $data) {
                ...
                if(!isset($options['depend']) || $options['depend']==true) $this->loadDependData($data);
    ...
    public function filter($datas,  $special = null, $trust = false, $select = null)
    {
        ...
        try {
            ...
            if(!isset($datas[PAGE_ACTIVE])) $datas[PAGE_ACTIVE] = 1;
            if(!isset($datas[PAGE_QUANTITY])) $datas[PAGE_QUANTITY] = 25;
            if(!isset($datas[PAGE_TOTAL])) $datas[PAGE_TOTAL] = 0;
            if(!isset($datas[FLAG_FILTER_CHANGE])) $datas[FLAG_FILTER_CHANGE] = 1;
    ...
    public function is_exist($keys, $special = null, $trust = false)
    {
        ...
        return count($dataBuilder->limit(1)->get()) > 0;
        // return $dataBuilder->limit(1)->count() > 0;
    }
