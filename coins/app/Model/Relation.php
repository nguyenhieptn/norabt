    ...
    public function add($newData){
        ...
        elseif($this->on_add === 'deny'){
            if($this->model->is_exist($dataKey)){
                Reply::finish(false, 'ERROR_DB', ['data'=>'Edit '.$this->model->name.' first']);
            }
            ...
            Reply::finish(false, 'ERROR_DB', ['data'=>'Define '.$this->model->name.' on Add']);
        }
    }
        ...
        if(count($deleteKey) > 0){
            ...
            if($this->on_delete==='cascade'){
                ...
                if($this->model->is_exist($deleteKey)){
                    Reply::finish(false, 'ERROR_DB', ['data'=>'Remove '.$this->model->name.' first']);
                }
                ...
                Reply::finish(false, 'ERROR_DB', ['data'=>'Define '.$this->model->name.' on Delete']);
            }
        }
    }
    ...
    public function edit($editDatas, $newData){
        ...
        if(count($dataKey) > 0){
            ...
            elseif($this->on_update === 'deny'){
                ...
                Reply::finish(false, 'ERROR_DB', ['data'=>'Define '.$this->model->name.' on Update']);
            }
