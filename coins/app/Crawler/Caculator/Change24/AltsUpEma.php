...
class AltsUpEma
{
    ...
    public static function caculate($N, &$startObjectIndex)
    {
        ...
        if (!isset($startObjectIndex[$index]) || $startObjectIndex[$index] == null) {
            ...
            $columnName = [
                'CHANGE24H_UP_EMA' => 'change24h_up_ema' . $N,
                'CHANGE24H_UP_EMA5' => 'change24h_up_ema5',
                'CHANGE24H_UP_EMA9' => 'change24h_up_ema9',
                'CHANGE24H_UP_EMA13' => 'change24h_up_ema13'
            ];

            $firstEmas = $model->read([[[$columnName['CHANGE24H_UP_EMA'], '>', 0]]], function ($db) use ($N, $columnName) {
            ...
            if (!isset($startObjectIndex[$index])) {
                ...
                $firstEmas = $model->read([[[CHANGE24H_UP, '>', 0]]], function ($db) use ($N) {
                ...
                if (count($firstEmas) == $N) {
                    ...
                    $model->edit([
                        ...
                        DATA_EDITOR => [$columnName['CHANGE24H_UP_EMA'] => $firstEma]
                    ]);

                    $startObjectIndex[$index]->{$columnName['CHANGE24H_UP_EMA']} = $firstEma;
                }
            }
        }
        ...
        $firstEma = $startObjectIndex[$index]->{$columnName['CHANGE24H_UP_EMA']};
        ...
        while (true) {
            $restPoints = $model->read([[[CHANGE24H_TIME, '>', $startPoint], [$columnName['CHANGE24H_UP_EMA'], '=', null]]], function ($db) use ($columnName) {
                $db->orderBy(CHANGE24H_TIME, 'ASC');
            }, true, [CHANGE24H_ID, CHANGE24H_TIME, $columnName['CHANGE24H_UP_EMA'], CHANGE24H_UP]);
            ...
            foreach ($restPoints as $restPoint) {
                $ema = $periodEma * (1 - $K) + $restPoint->{CHANGE24H_UP} * $K;


                $result = $model->edit([
                    DATA_KEY => [[[CHANGE24H_ID, '=', $restPoint->{CHANGE24H_ID}]]],
                    DATA_EDITOR => [$columnName['CHANGE24H_UP_EMA'] => $ema]
                ]);
                if (!$result['result']) return $result;
                $restPoint->{$columnName['CHANGE24H_UP_EMA']} = $ema;
