...
class CandleRealtimeRedis
{
    ...
    private function catchEvent($streamData)
    {
        ...
        if (json_last_error() == JSON_ERROR_NONE) {
            ...
            Redis::setex("candle_data_" . $this->symbol, 60, json_encode($this->redisData));
    ...
    public function processData($candle, $frame)
    {
        ...
        if ($frame == '1m') {
            ...
            $this->redisData[$frame] = $this->array_last($this->data[$frame]['data'], $this->redisDataLeng);
            ...
            if($frame == '1h' || $frame == '4h' || $frame == '1d' || $frame == '1w'){
                ...
                IndexCalculate::calRsiWma($this->data[$frame]['data'], $updateData, $frame, 45, 'close', 'price_wma');
                ...
                IndexCalculate::calEma($periodData, $updateData, $frame, 9, 'close', 'price_ema');
                IndexCalculate::calEma($periodData, $updateData, $frame, 9, 'rsi14', 'rsi_ema');
    ...
    private function array_last($array, $leng){
        $result = [];
        $len = count($array);
        for($i = $len - 1; $i >= $len - $leng; $i--){
            if($i < 0) break;
            $result[] = $array[$i];
        }
        return $result;

        // return array_reverse(array_splice($array, -$leng, $leng));
