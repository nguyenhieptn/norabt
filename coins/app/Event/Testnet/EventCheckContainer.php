...
class EventCheckContainer
{
    ...
    function __construct($campaignId)
    {
        ...
        try {
            ...
            $this->accountModel = Models::get('Admin/Testnet_account');
            ...
            $account = $this->accountModel->read([[[TESTNET_ACCOUNT_ID, '=', $campaign->{TESTNET_ACCOUNT}]]]);
            if (!$account['result'] || !isset($account['data'][0])) {
                throw new Exception('No Account ID ' . $campaign->{TESTNET_ACCOUNT});
            }
            ...
            if ($type != 0) {
                ...
                if ($time - $LabPendingTime >= ($interval * 60000)) {
                    ...
                    $addData = [
                        ...
                        TESTNET_RESULT_MATCHED_QTY => 0,
                        TESTNET_RESULT_MATCHED_PRICE => 0,
                        TESTNET_RESULT_ACCOUNT => $this->account->{TESTNET_ACCOUNT_ID},
                    ];
                    ...
                    Telegram::send(
                        TELE_ICON_WAITTING . " [" .  $this->campaign->{TESTNET_NAME} . "] #" . $this->symbol
    ...
    private function checkEvent()
    {
        ...
        //Get Event Data
        if ($this->event) {
            if ($this->event->{TESTNET_RESULT_PENDING} == 0) {
                return Reply::make(true, 'No Pending Event', false);
            }
        }
        ...
        if ($status == TESTNET_RESULT_STATUS_PENDING || $status == TESTNET_RESULT_STATUS_PHASE_PENDING) {
            ...
            if ($type == TESTNET_RESULT_TYPE_LONG) {
                ...
                if ($market || $price <= $enterPrice) {
                    ...
                    $editData = [
                        TESTNET_RESULT_STATUS => $nextPhase == ($this->getMatchLeng($flowName) - 1) ? TESTNET_RESULT_STATUS_MATCHED : TESTNET_RESULT_STATUS_MATCHED_PART,
                        ...
                        TESTNET_RESULT_BASEPROFIT => doubleval(get($condition['baseprofit'], $this->getFlowData($flowName, 'baseprofit'))),
            ...
            if (!$isMatched) {
                ...
                if ((doubleval($time) - $startTime) >= $timelife * 60000) {
                    ...
                    Telegram::send(
                        TELE_ICON_CANCEL . " [" . $this->campaign->{TESTNET_NAME} . "] #" . $this->symbol . " Cancel \n- Time:" . date("H:i:s d/m/Y", $time / 1000),
                        $this->teleGrNotice,
                        $this->teleBot
                    );
        ...
        if (doubleval($event->{TESTNET_RESULT_MATCHED_QTY}) > 0 && $status != TESTNET_RESULT_STATUS_STOP_PENDING) {
            ...
            //Check liquid
            if($this->account->{TESTNET_ACCOUNT_MARGIN_TYPE} != 'CROSS'){
                if($this->checkLiquidation()){
                    $editData = [
                        TESTNET_RESULT_STATUS => TESTNET_RESULT_STATUS_STOP_PENDING,
                        TESTNET_RESULT_PARAMS => 'Liquidation'
                    ];
                    $finished = true;
                }
            }
            ...
            if ($finished) {
                ...
                Telegram::send(
                    TELE_ICON_STOP . " [" . $this->campaign->{TESTNET_NAME} . "] #" . $this->symbol
        ...
        if ($status == TESTNET_RESULT_STATUS_STOP_PENDING) {
            ...
            $pnl = $pnlData['pnl'];
            $isProfit = $pnl > $commit;
            $package = $event->{TESTNET_RESULT_BUDGET};
            $realPNL = $pnl - $commit;
            $margin = get($event->{TESTNET_RESULT_MARGIN}, 1);
            ...
            $ms = ($isProfit ? TELE_ICON_TAKEPROFIT : TELE_ICON_STOPLOSS) . " " . $this->campaign->{TESTNET_NAME} . " - TỔNG KẾT"
                ...
                . "\n- Loại vị thế: " . ($type == TESTNET_RESULT_TYPE_LONG ? "Long" : "Short")
                . "\n- Flow: " . $flow
                . "\n- Margin Mode: Cross"
        ...
        if ($status == TESTNET_RESULT_STATUS_MATCHED_PART) {
            ...
            if ($phase < ($this->getMatchLeng($flowName) - 1)) {
                ...
                if ($checkResult['data']) {
                    ...
                    $baseonData = json_decode($event->{TESTNET_RESULT_BASE}, true);;
        ...
        if ($status == TESTNET_RESULT_STATUS_RELEASE_WAITTING) {
            ...
            $releaseCondition = $this->getStopCondition($flowName, $event->{TESTNET_RESULT_ORDER_RELEASE});
            ...
            if ($isRelease) {
                ...
                Telegram::send(
                    TELE_ICON_CANCEL . " [" . $this->campaign->{TESTNET_NAME} . "] #" . $this->symbol . " Cancel waitting Release Phase $phase \n- Time:" . date("H:i:s d/m/Y"),
                    $this->teleGrNotice,
                    $this->teleBot
                );
    ...
    private function makeOrder($action, $closePrice, $phase, $time)
    {
        ...
        if ($margin < doubleval($action->{TESTNET_RESULT_MARGIN})) {
        ...
        $this->updateEvent(
            [
                ...
                DATA_EDITOR => [
                    ...
                    TESTNET_RESULT_BUDGET => $this->campaign->{TESTNET_BUDGET}
                ]
            ],
            true
        );
    ...
    {
        if ($profit > 0) {
            $this->accountModel->db()->where(TESTNET_ACCOUNT_ID, '=', $this->account->{TESTNET_ACCOUNT_ID})->increment(TESTNET_ACCOUNT_BALANCE, $profit);
        } else {
            $this->accountModel->db()->where(TESTNET_ACCOUNT_ID, '=', $this->account->{TESTNET_ACCOUNT_ID})->decrement(TESTNET_ACCOUNT_BALANCE, -$profit);
        }
    }
    ...
    private function autoArrange($profit = 0)
    {
        ...
        $account = $this->accountModel->read([[[TESTNET_ACCOUNT_ID, '=', $this->account->{TESTNET_ACCOUNT_ID}]]]);
        ...
        if ($account->{TESTNET_ACCOUNT_COMPOUND} == 1) {

            $campaigns = $this->campaignModel->read([[[TESTNET_ACCOUNT, '=', $account->{TESTNET_ACCOUNT_ID}]]]);
            if (!$campaigns['result']) return $campaigns;
            $campaigns = $campaigns['data'];

            $reserve = get($account->{TESTNET_ACCOUNT_RESERVE}, 0);

            $used = 0;

            foreach ($campaigns as $campaign) {
                $used += doubleval($campaign->{TESTNET_BUDGET});
            }

            $balance = doubleval($account->{TESTNET_ACCOUNT_BALANCE});
            $balance = $balance * (100 - $reserve) / 100;

            $free = $balance - $used;

            // if ($free <= 0) {
            //     $ms = TELE_ICON_WARNING . " [" . $this->campaign->{TESTNET_NAME} . "] auto arrange fail"
            //         . "\n- Symbol: " . $this->symbol
            ...
            foreach ($campaigns as $campaign) {
                $result = $this->campaignModel->edit([
                    DATA_KEY => [[[TESTNET_ID, '=', $campaign->{TESTNET_ID}]]],
                    DATA_EDITOR => [TESTNET_BUDGET => doubleval($campaign->{TESTNET_BUDGET}) + $added]
                ]);
                if (!$result['result']) return $result;
            }

            $ms = TELE_ICON_WARNING . " [" . $this->campaign->{TESTNET_NAME} . "] auto arrange successfully"
                . "\n- Symbol: " . $this->symbol
                . "\n- Free: " . round($free * 1000) / 1000 . " $"
            ...
            if ($this->campaign->{TESTNET_COMPOUND} == 1) {

                if ($profit > 0) {
                    $this->campaignModel->db()->where(TESTNET_ID, '=', $this->campaign->{TESTNET_ID})->increment(TESTNET_BUDGET, $profit);
                } else {
                    $this->campaignModel->db()->where(TESTNET_ID, '=', $this->campaign->{TESTNET_ID})->decrement(TESTNET_BUDGET, -$profit);
                }

                $ms = TELE_ICON_WARNING . " [" . $this->campaign->{TESTNET_NAME} . "] update budget successfully"
                    . "\n- Symbol: " . $this->symbol
    ...
    private function getInvestBudget()
    {
        ...
        $campaignData = $this->campaignModel->read([[[TESTNET_ID, '=', $this->campaign->{TESTNET_ID}]]]);
        ...
        return doubleval($campaignData->{TESTNET_BUDGET}) * doubleval($campaignData->{TESTNET_ACTIVE_BUDGET}) / 100;
    }
    ...
    {
        $flow = $event->{TESTNET_RESULT_FLOW};
        $strategy = $event->{TESTNET_RESULT_STRATEGY};
    ...
    private function countSlot($flowName)
    {
        ...
        $openTrades = $this->eventModel->count([[
            [TESTNET_RESULT_PENDING, '=', 1],
            [TESTNET_RESULT_ACCOUNT, '=', $this->account->{TESTNET_ACCOUNT_ID}],
            [TESTNET_RESULT_STRATEGY, '=', $strategyId]
        ]]);
    ...
    {
        $campaigns = $this->campaignModel->read([[[TESTNET_ACCOUNT, '=', $this->account->{TESTNET_ACCOUNT_ID}]]]);
        if (!$campaigns['result']) return $campaigns;
        $campaigns = $campaigns['data'];
        $priorityIndex = [];
        foreach ($campaigns as $campaign) {
            $priorityIndex[$campaign->{TESTNET_SYMBOL}] = doubleval($campaign->{TESTNET_PRIORITY});
        }
    ...
    private function checkPositionPriority($flowName, $freeSlot = null)
    {
        ...
        foreach ($waittingTrades as $waitting) {
            $symbol = $waitting->{TESTNET_RESULT_SYMBOL};
    ...
    private function checkBlacklist($flowName)
    {
        ...
        $symbol = $this->campaign->{TESTNET_SYMBOL};
    ...
    private function checkLiquidation(){
        ...
        $low = doubleval($khung1m[0]->{CANDLE_1M_LOW});
        $event = $this->event;
        $matchedQty = doubleval($event->{TESTNET_RESULT_MATCHED_QTY});
        $matchedPrice = doubleval($event->{TESTNET_RESULT_MATCHED_PRICE});
        $margin = doubleval($event->{TESTNET_RESULT_MARGIN});
        if($margin == 0) $margin = 1;


        
        $invest = $matchedQty * $matchedPrice/$margin;
        if($event->{TESTNET_RESULT_TYPE} == TESTNET_RESULT_TYPE_LONG){
            $worstPrice = $low;
