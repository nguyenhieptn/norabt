...
class FileFunc {
    ...
    public static function read($uploader, $disk, $table, $column, $uid, $file){
        $action = 'Read';
        if(!Checker::validate([$disk, $column, $uid], 'Word')) Reply::finish(false, 'Wrong format file name');
        $path = $table.'/'.$column.'/'.$uid;
    ...
    public static function get($table, $column = null, $uid = null, $special = null){
    
        $condition = [[FILE_TABLE, '=', $table]];
        if($column != null) $condition[] = [FILE_COLUMN,'=', $column];
        if($uid != null) $condition[] = [FILE_UID,'=', $uid];
    
        $files = Models::get('Uploader/Uploader_files')->read([$condition], $special);
        return $files;
    }
    ...
    public static function file_public($path){
        if(!$path) return '';
        return preg_replace('/(https?:\/\/[^\/]+)/', "$1/api/uploader/public/read?file=$1", $path);
    } 
