...
class Lab_candle_4hController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Lab_candle_4h');
        $this->mainModel->query_builder = DB::connection(Ctrl::get('control_lab_db', 'coin_crawler'))->table('lab_candle_4h');
        $this->tableName = LAB_CANDLE_4H_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[LAB_CANDLE_4H_ID] = true;
    ...
    public function addGetIds(Request $request)
    {
        ...
        foreach ($datas as $key=>$val){
            $datas[$key] = array_diff_key($datas[$key], $this->dependCols);
            $datas[$key][LAB_CANDLE_4H_ID] = uniqid();
        }
        $result = $this->mainModel->add($datas);
        if(!$result['result']) Reply::finish($result);
        Reply::finish(true, 'success', $datas);
    ...
    public function mapping(){
        $mapData = [];
        $mapData[LAB_CANDLE_4H_SYMBOL] = Edge::mapping(Models::get('Admin/Lab_watchlist'), null, LAB_WL_SYMBOL, LAB_WL_SYMBOL);
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[LAB_CANDLE_4H_TIME, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{LAB_CANDLE_4H_ID} => $item->{LAB_CANDLE_4H_TIME}];
            });
        }
    ...
    public function sort(Request $request){
        ...
        $srcData = $this->mainModel->read([[[LAB_CANDLE_4H_ID, '=', $sourceID]]]);
        if(isset($srcData['data'][0])) $srcData = $srcData['data'][0];
        $destData = $this->mainModel->read([[[LAB_CANDLE_4H_ID, '=', $destID]]]);
        if(isset($destData['data'][0])) $destData = $destData['data'][0];
    
        if($srcData->{LAB_CANDLE_4H_SYMBOL} > $destData->{LAB_CANDLE_4H_SYMBOL}){
            $maxW = $srcData->{LAB_CANDLE_4H_SYMBOL};
            $minW = $destData->{LAB_CANDLE_4H_SYMBOL};
            $vector = 1;
        }else{
            $minW = $srcData->{LAB_CANDLE_4H_SYMBOL};
            $maxW = $destData->{LAB_CANDLE_4H_SYMBOL};
            $vector = 0;
        }
    
        $sortProjects = $this->mainModel->read( [[[LAB_CANDLE_4H_SYMBOL, '>=', $minW], [LAB_CANDLE_4H_SYMBOL, '<=', $maxW]]], function($db){$db->orderBy(LAB_CANDLE_4H_SYMBOL, 'desc');});
        ...
        if($vector==1){
            ...
            $this->mainModel->edit( [
                DATA_KEY => [[[LAB_CANDLE_4H_ID, '=', $sourceID]]],
                DATA_EDITOR => [LAB_CANDLE_4H_SYMBOL=>0]
            ]);
    
            foreach ($sortProjects as $key=>$sortProject){
                if(isset($sortProjects[$key-1])){
                    $this->mainModel->edit( [
                        DATA_KEY => [[[LAB_CANDLE_4H_ID, '=', $sortProject->{LAB_CANDLE_4H_ID}]]],
                        DATA_EDITOR => [LAB_CANDLE_4H_SYMBOL=>$sortProjects[$key-1]->{LAB_CANDLE_4H_SYMBOL}]
                        ]);
                }
            ...
            $this->mainModel->edit( [
                DATA_KEY => [[[LAB_CANDLE_4H_ID, '=', $sourceID]]],
                DATA_EDITOR => [
                    LAB_CANDLE_4H_SYMBOL=>$destData->{LAB_CANDLE_4H_SYMBOL},
            ...
            for ($key = count($sortProjects)-1; $key >= 0; $key--){
                ...
                if(isset($sortProjects[$key+1])){
                    $this->mainModel->edit( [
                        DATA_KEY => [[[LAB_CANDLE_4H_ID, '=', $sortProjects[$key]->{LAB_CANDLE_4H_ID}]]],
                        DATA_EDITOR => [LAB_CANDLE_4H_SYMBOL=>$sortProjects[$key+1]->{LAB_CANDLE_4H_SYMBOL}]
                        ]);
                }
