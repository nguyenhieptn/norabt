...
class Lab_orderController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Lab_order');
        $this->tableName = LAB_ORDER_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[LAB_ORDER_ID] = true;
    ...
    public function mapping(){
        $mapData = [];
        $mapData[LAB_ORDER_TYPE] = [LAB_RESULT_TYPE_LONG => 'Long', LAB_RESULT_TYPE_SHORT => 'Short'];
    ...
    public function filter(Request $request)
    {
        ...
        $id = get($datas[LAB_ORDER_ACTION], null);

        $responseData = $this->mainModel->filter($datas, function($db) use($id){
            $db->where(LAB_ORDER_ACTION, '=', $id);
        });
