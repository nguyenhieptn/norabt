...
class LabController extends Controller  
{
    ...
    public function read(Request $request)
    {
        $datas = $request->all();
        $datas = $this->mainModel->keyToCondition($datas);
        $readResult = $this->mainModel->read([$datas], null, false, [
            LAB_ID, LAB_TIME, LAB_TYPE, LAB_PROFIT, LAB_STATUS, LAB_SYMBOL, LAB_PENDING
        ]);
    ...
    public function mapping(){
        $mapData = [];
        $mapData[LAB_TYPE] = (object)[LAB_TYPE_LONG => 'Long', LAB_TYPE_SHORT => 'Short'];
        $mapData[LAB_STATUS] = (object)[LAB_STATUS_PENDING => 'Pending', LAB_STATUS_MATCHED => 'Matched', LAB_STATUS_STOPLOSS=>'Stop Loss', LAB_STATUS_TAKEPROFIT => 'Take Profit', LAB_STATUS_CANCLE => 'Cancle'];
        $mapData[LAB_PENDING] = (object)[0 => 'Finished', 1 => 'Opening'];
        $mapData[LAB_SYMBOL] = Edge::mapping(Models::get('Admin/Watchlist'), null, WL_SYMBOL, WL_SYMBOL);
        $mapData[LAB_STRATEGY] = Edge::mapping(Models::get('Admin/Strategies'), null, STRATEGY_ID, STRATEGY_NAME);
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[LAB_SYMBOL, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{LAB_ID} => $item->{LAB_SYMBOL}];
            });
        }
    ...
    public function filter(Request $request)
    {
        ...
        $symbol = $datas['symbol'];
        $responseData = $this->mainModel->filter($datas, function($db) use($symbol){
            // $db->where(LAB_SYMBOL, '=', $symbol);
        });
    ...
    public function get(Request $request){
        ...
        $result = $this->mainModel->read($data, function($db) use($limit, $skip, $orderBy){
            if($orderBy !== null) $db->orderBy($orderBy);
            if($skip !== null) $db->skip($skip);
            if($limit !== null) $db->limit($limit);
        }, false, [
            LAB_ID, LAB_TIME, LAB_TYPE, LAB_PROFIT, LAB_STATUS, LAB_SYMBOL, LAB_PENDING, LAB_PARAMS, LAB_STRATEGY
        ]);
