...
class Change_24hController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Change_24h');
        $this->tableName = CHANGE_24H_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[CHANGE24H_ID] = true;
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[CHANGE24H_TIME, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{CHANGE24H_ID} => $item->{CHANGE24H_TIME}];
            });
        }
    ...
    public function get(Request $request){
        ...
        $selectColumn = $request->input('selected', null);

        if($selectColumn == null){
            $selectColumn = array_keys($this->mainModel->struct);
            $selectColumn = array_diff($selectColumn, [CHANGE24H_DATA]);
        }
