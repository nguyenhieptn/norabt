...
class Candle_1dController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Candle_1d');
        $this->tableName = CANDLE_1D_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[CANDLE_1D_ID] = true;
    ...
    public function mapping(){
        $mapData = [];
        $mapData[CANDLE_1D_SYMBOL] = Edge::mapping(Models::get('Admin/Watchlist'), null, WL_SYMBOL, WL_SYMBOL);
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[CANDLE_1D_SYMBOL, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{CANDLE_1D_ID} => $item->{CANDLE_1D_SYMBOL}];
            });
        }
    ...
    public function clearColumn(Request $request){
        ...
        $result = $this->mainModel->edit([
            DATA_KEY => [[[CANDLE_1D_SYMBOL, '=', $symbol]]],
