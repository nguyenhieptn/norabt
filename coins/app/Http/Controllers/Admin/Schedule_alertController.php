...
class Schedule_alertController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Schedule_alert');
        $this->tableName = SCHEDULE_ALERT_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[SCHEDULE_AL_ID] = true;
    ...
    public function addGetIds(Request $request)
    {
        ...
        foreach ($datas as $key=>$val){
            $datas[$key] = array_diff_key($datas[$key], $this->dependCols);
            $datas[$key][SCHEDULE_AL_ID] = uniqid();
        }
        $result = $this->mainModel->add($datas);
        if(!$result['result']) Reply::finish($result);
        Reply::finish(true, 'success', $datas);
    ...
    public function mapping(){
        $mapData = [];
        $mapData[SCHEDULE_AL_ICON] = ['↗️' => '↗️', '↘️' => '↘️', '✅' => '✅', '⛔️' => '⛔️', '⏹' => '⏹', '🚫' => '🚫', '🤝' => '🤝', '🕒' => '🕒', '⚠️'=>'⚠️'];
        $mapData[SCHEDULE_AL_DONE] = (object)['0' => 'Active', '1' => 'Done'];
        $mapData[SCHEDULE_AL_BOTID]=Edge::mapping($this->mainModel, null, SCHEDULE_AL_BOTID, SCHEDULE_AL_BOTID, null, function($db){
            $db->distinct();
        });
        $mapData[SCHEDULE_AL_GROUPID]=Edge::mapping($this->mainModel, null, SCHEDULE_AL_GROUPID, SCHEDULE_AL_GROUPID, null, function($db){
            $db->distinct();
        });
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[SCHEDULE_AL_NAME, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{SCHEDULE_AL_ID} => $item->{SCHEDULE_AL_NAME}];
            });
        }
