...
class Candle_4hController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Candle_4h');
        $this->tableName = CANDLE_4H_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[CANDLE_4H_ID] = true;
    ...
    public function mapping(){
        $mapData = [];
        $mapData[CANDLE_4H_SYMBOL] = Edge::mapping(Models::get('Admin/Watchlist'), null, WL_SYMBOL, WL_SYMBOL);
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[CANDLE_4H_SYMBOL, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{CANDLE_4H_ID} => $item->{CANDLE_4H_SYMBOL}];
            });
        }
    ...
    public function getTrend3(Request $request)
    {

        $responseData = $this->mainModel->read([[[CANDLE_4H_SYMBOL, '=', 'BTCUSDT']]], function ($db) {
            $db->orderBy(CANDLE_4H_CLOSE_TIME, 'DESC')->limit(1);
        });
        if(!$responseData['result']) Reply::finish($responseData);
        Reply::finish($responseData);
