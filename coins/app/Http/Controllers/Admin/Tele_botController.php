...
class Tele_botController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Tele_bot');
        $this->tableName = TELE_BOT_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[TELE_BOT_ID] = true;
    ...
    public function mapping(){
        $mapData = [];
        $mapData[TELE_BOT_ROLE] = [TELE_BOT_ROLE_DEFAULT => 'Default' ];
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[TELE_BOT_NAME, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{TELE_BOT_ID} => $item->{TELE_BOT_NAME}];
            });
        }
