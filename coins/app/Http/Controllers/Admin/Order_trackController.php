...
class Order_trackController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Order_track');
        $this->tableName = ORDER_TRACK_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[ORDER_TRACK_ID] = true;
    ...
    public function addGetIds(Request $request)
    {
        ...
        foreach ($datas as $key=>$val){
            $datas[$key] = array_diff_key($datas[$key], $this->dependCols);
            $datas[$key][ORDER_TRACK_ID] = uniqid();
        }
        $result = $this->mainModel->add($datas);
        if(!$result['result']) Reply::finish($result);
        Reply::finish(true, 'success', $datas);
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[ORDER_TRACK_SYMBOL, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{ORDER_TRACK_ID} => $item->{ORDER_TRACK_SYMBOL}];
            });
        }
    ...
    {
        $datas = $request->all();
        Ctrl::set('order_track_active_time', time());
    ...
    public function reloadAlert(){
        exec('sudo systemctl restart order_track');
        Reply::finish(true, 'success');
    }
    public function reloadOrderAlert(){
        exec('sudo systemctl restart order_alert');
        Reply::finish(true, 'success');
    }
