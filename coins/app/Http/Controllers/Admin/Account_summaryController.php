...
class Account_summaryController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Account_summary');
        $this->tableName = ACCOUNT_SUMMARY_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[AC_SUM_ID] = true;
    ...
    public function mapping(){
        $mapData = [];
        $mapData[AC_SUM_ACCOUNT] = Edge::mapping(Models::get('Admin/Accounts'), null, ACCOUNT_ID, ACCOUNT_NAME);
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[AC_SUM_ACCOUNT, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{AC_SUM_ID} => $item->{AC_SUM_ACCOUNT}];
            });
        }
    ...
    public function calculate(){
        $command = 'sudo php ' . base_path() . '/artisan account_summary';
        $result = exec($command);
