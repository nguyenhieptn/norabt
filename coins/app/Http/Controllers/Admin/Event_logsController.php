...
class Event_logsController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Event_logs');
        $this->tableName = EVENT_LOGS_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[ELOG_ID] = true;
    ...
    public function mapping(){
        $mapData = [];
        $mapData[ELOG_MATCHED] = (object)[0 => 'Not Matched', 1=>'Long', 2=>'Short', 3 => 'Both'];
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[ELOG_SYMBOL, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{ELOG_ID} => $item->{ELOG_SYMBOL}];
            });
        }
