...
class Candle_1mController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Candle_1m');
        $this->tableName = CANDLE_1M_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[CANDLE_1M_ID] = true;
    ...
    public function mapping(){
        $mapData = [];
        $mapData[CANDLE_1M_SYMBOL] = Edge::mapping(Models::get('Admin/Watchlist'), null, WL_SYMBOL, WL_SYMBOL);
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[CANDLE_1M_SYMBOL, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{CANDLE_1M_ID} => $item->{CANDLE_1M_SYMBOL}];
            });
        }
    ...
    public function clearColumn(Request $request){
        ...
        $result = $this->mainModel->edit([
            DATA_KEY => [[[CANDLE_1M_SYMBOL, '=', $symbol]]],
