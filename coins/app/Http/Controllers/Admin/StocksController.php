...
class StocksController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Stocks');
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[STOCK_SYMBOL, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{STOCK_ID} => $item->{STOCK_SYMBOL}];
            });
        }
