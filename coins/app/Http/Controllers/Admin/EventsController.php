...
class EventsController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Events');
    ...
    public function mapping(){
        $mapData = [];
        $mapData[EVENT_TYPE] = (object)[1=>'Long', 2=>'Short'];
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[EVENT_SYMBOL, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{EVENT_ID} => $item->{EVENT_SYMBOL}];
            });
        }
