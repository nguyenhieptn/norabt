...
class Lab_event_logsController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Lab_event_logs');
        $this->tableName = LAB_EVENT_LOGS_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[LAB_ELOG_ID] = true;
    ...
    public function mapping(){
        $mapData = [];
        $mapData[LAB_ELOG_SYMBOL] = Edge::mapping(Models::get('Admin/Lab_watchlist'), null, LAB_WL_SYMBOL, LAB_WL_SYMBOL);
        $mapData[LAB_ELOG_CAMPAIGN] = Edge::mapping(Models::get('Admin/Lab_campaigns'), null, LAB_CAMPAIGN_ID, LAB_CAMPAIGN_NAME);
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[LAB_ELOG_SYMBOL, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{LAB_ELOG_ID} => $item->{LAB_ELOG_SYMBOL}];
            });
        }
    ...
    public function getProfit(Request $request){
        ...
        if($campaign == null) return Reply::make(false, 'No campaign');
       
        $result = $this->mainModel->read([[[LAB_ELOG_CAMPAIGN, '=', $campaign], [LAB_ELOG_MATCHED, '>', '0']]], null, false, [LAB_ELOG_TIME, LAB_ELOG_MAXPROFIT, LAB_ELOG_MINPROFIT, LAB_ELOG_PROFIT, LAB_ELOG_BASEPROFIT]);
        if(!$result['result']) return $result;
        $result = $result['data'];

        $finishFlag = $this->mainModel->read([[[LAB_ELOG_CAMPAIGN, '=', $campaign], [LAB_ELOG_STATUS, '=', 'Stop Pending']]], null, false, [LAB_ELOG_TIME, LAB_ELOG_MAXPROFIT, LAB_ELOG_MINPROFIT, LAB_ELOG_PROFIT, LAB_ELOG_BASEPROFIT]);
        if(!$finishFlag['result']) return $finishFlag;
        $finishFlag = $finishFlag['data'];

        return Reply::finish(true, 'success', ['profit' => $result, 'finish' => $finishFlag]);
    }
