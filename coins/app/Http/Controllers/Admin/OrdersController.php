...
class OrdersController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Orders');
    ...
    public function mapping(){
        $mapData = [];
        $mapData[ORDER_SYMBOL] = Edge::mapping(Models::get('Admin/Watchlist'), null, WL_SYMBOL, WL_SYMBOL);
        $mapData[ORDER_ACCOUNT] = Edge::mapping(Models::get('Admin/Accounts'), null, ACCOUNT_ID, ACCOUNT_NAME);
        $mapData[ORDER_SIDE] = ['SELL' => 'SELL', 'BUY' => 'BUY'];
        $mapData[ORDER_TYPE] = ['MARKET' => 'MARKET', 'LIMIT'=>'LIMIT', 'STOP_MARKET' => 'STOP_MARKET', 'TAKE_PROFIT_MARKET' => 'TAKE_PROFIT_MARKET'];
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[ORDER_BINANCE, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{ORDER_ID} => $item->{ORDER_BINANCE}];
            });
        }
    ...
    public function filter(Request $request)
    {
        ...
        $action = get($datas[ORDER_ACTION], null);
        $responseData = $this->mainModel->filter($datas, function($db)use($action){
            if($action) $db->where(ORDER_ACTION, '=', $action);
        });
