...
class Lab_nodeController extends Controller  
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = Models::get('Admin/Lab_node');
        $this->tableName = LAB_NODE_TABLE;
        $this->mainModel->loadDepend();
        $this->dependCols = array_unique(array_column($this->mainModel->registerDepend, 1, 1));
        $this->dependCols[LAB_NODE_ID] = true;
    ...
    public function suggest(Request $request)
    {
        ...
        $suggestData = $this->mainModel->read([[[LAB_NODE_NAME, 'contain', $datas]]], function($builder){$builder->limit(20);});
        $suggest = [];
        if($suggestData['result']){
             
            $suggest = $suggestData['data']->mapWithKeys(function ($item){
                return [$item->{LAB_NODE_ID} => $item->{LAB_NODE_NAME}];
            });
        }
    ...
    {
        $server = $request->input('server', 'Localhost');
        $sysInfo = LabSocket::makeRemoteQuery($server, 'system', 'getInfo');
        if ($sysInfo['result']) {
            $data = $sysInfo['data'];
            $editData = [
