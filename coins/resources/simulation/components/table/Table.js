...
class Table extends Component {
	...
	constructor(props) {
		...
		this[STRUCT_TABLE][PAGE_QUANTITY] = get(localStorage.getItem("page_quantity_" + this[STRUCT_TABLE][DATA_TABLE_ID]), this[STRUCT_TABLE][PAGE_QUANTITY])
	    ...
	    this.model.filter(filterData, loading, this[STRUCT_TABLE][DATA_SPECIAL]).then((response)=>{
			if(response){
				if(response['result']){
					...
					if(this.props.onFilterSuccess) this.props.onFilterSuccess(data , response['message']);
	...
	map(loading=true){
		return this.model.map(loading).then((res)=>{
			if(res){
				if(res['result']){
