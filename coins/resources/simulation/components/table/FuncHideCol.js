	...
	render() {
		...
		return (
			<div className="dropdown" id={this.id}>
				<button className="button btn btn-sm btn-primary dropdown-toggle" type="button" data-toggle="dropdown" style={{ marginLeft: 0 }}>{lang('Columns')}
					<span className="caret"></span></button>
				<ul className="dropdown-menu" style={{ padding: 5, top: 60 }}>
					<li><div className="button btn btn-primary" style={{ marginBottom: '10px' }} onClick={() => { this.onDefault() }}>{lang('Default')}</div></li>
					{this.drawColSel()}
					{/* <li><div className="button btn btn-primary" onClick = {()=>{this.onDefault()}}>{lang('Default')}</div></li>  */}
				</ul>
