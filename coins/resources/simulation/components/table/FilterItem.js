...
class FilterItem extends Component {
	...
	drawInput() {
		...
		switch (this.struct[FILTER_TYPE]) {
			...
			case 'select_unsort': {
				this.input_truct[INPUT_TYPE] = 'select'
				return (
					<>
						<div className={`filter_item ${this.props.className}`}>
							{this.props.text === false ? '' : <div className="filter_text" title={this.struct[FILTER_NAME]}>{this.struct[FILTER_NAME]}</div>}
							<div className="filter_input">
								{this.logic.map(item => {
									return <Input sort={false} onFocus={()=>{this.isEditting = true}} onBlur={()=>{this.isEditting = false}} key={item} ref={input => this.inputComps[item] = input} className="filter_item_input" struct={this.input_truct} onChange={this.onChangeBlur} {...this.rent}></Input>
								})}
							</div>
						</div>
					</>
