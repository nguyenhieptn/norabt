...
class ChartStopLoss extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    async getData() {
        ...
        if (res['result']) {
            ...
            var total = [];
            res.map(item => {
                var time = item[LAB_RESULT_SELL_TIME];
                if (time < this.state.stop && time > this.state.start) {
                    if (Number(item[LAB_RESULT_STATUS]) == 3) {
                        var index = coin.indexOf(item[LAB_RESULT_SYMBOL]);

                        var realPNL = Number(item[LAB_RESULT_REAL_PNL]);
