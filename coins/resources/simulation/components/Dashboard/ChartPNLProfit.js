...
class ChartPNLProfit extends Component {
            ...
            <>
                <div className='p-col-12 p-md-12 ' >
                    <div className='box_shadow ' ref={c => this.chartContainer = c} style={{ position: 'relative' }}>
                        {!App.isMobile() &&
                            <div className='cus-hide' style={{ display: 'flex', alignItems: 'center', padding: 7, position: 'absolute', zIndex: 1, top: '35px', right: '65px' }}>
        ...
        resultModel.getByAccount().then(res => {
            if (res['data']) {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    var item = data[i];
                    var per = Math.round((total) * 100) / 100;
                    data1.push({
                        x: item.x,
                        y: per
                    })
