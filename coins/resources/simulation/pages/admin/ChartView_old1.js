...
class ChartView extends Component {
	...
	getAllData(chart = null) {
		if(chart === null){
			this.getData().then(res => {
				return this.getData(true, 1000, true, 1);
			}).then(res => {
				return this.getData(true, 1000, true, 2);
			}).then(res => {
				this.setState({
			...
			this.getData(true, 1000, true, chart).then(()=>{
				if(chart == 1){
					this.setState({
	...
	componentDidMount() {
		...
		if(App.strategySelector){
			App.strategySelector.register('testnes_chart', ()=>{
				this.getAllData();
			});
		}
 ...
 */
	...
	componentWillUnmount() {
		...
		App.strategySelector.unregister('testnes_chart')
	}
	...
	getData(loading = true, limit = 1000, reset = true, chart=0) {
		...
		return axios.request({
			...
			data: {
				...
				interval: frame,
				limit: limit,
				strategy: App.strategySelector.selected(),
			}
		})
			...
			.then(response => {
				...
				if (response['result']) {
					...
					for (let i in labFlag) {
						var fillColor = "white";
						if (labFlag[i][LAB_RESULT_STATUS] == LAB_RESULT_STATUS_MATCHED) fillColor = 'orange';
						else if (labFlag[i][LAB_RESULT_STATUS] == LAB_RESULT_STATUS_TAKEPROFIT) fillColor = 'green';
						else if (labFlag[i][LAB_RESULT_STATUS] == LAB_RESULT_STATUS_STOPLOSS) fillColor = 'red';
						else if (labFlag[i][LAB_RESULT_STATUS] == LAB_RESULT_STATUS_CANCLE) fillColor = 'gray';
						labFlagData.push(
							{
								x: get(labFlag[i][LAB_RESULT_MATCHED_TIME], labFlag[i][LAB_RESULT_ORDER_TIME]),      // Point where the labFlag appears
								y: get(labFlag[i][LAB_RESULT_MATCHED_PRICE], labFlag[i][LAB_RESULT_ORDER_PRICE]),
								title: labFlag[i][LAB_RESULT_TYPE] == '1' ? 'Lab Long' : 'Lab Short', // Title of labFlag displayed on the chart 
								text: labFlag[i][LAB_RESULT_ORDER_PRICE],  // Text displayed when the labFlag are highlighted.
								fillColor: fillColor,
								profit: Number(labFlag[i][LAB_RESULT_PROFIT]),
								price: labFlag[i][LAB_RESULT_ORDER_PRICE],
								time: moment(labFlag[i][LAB_RESULT_ORDER_TIME], 'x').format('HH:mm:ss'),
								matched: labFlag[i][LAB_RESULT_MATCHED_PRICE],
								matched_time: moment(labFlag[i][LAB_RESULT_MATCHED_TIME], 'x').format('HH:mm:ss'),
								sell: labFlag[i][LAB_RESULT_SELL_PRICE],
								sell_time: moment(labFlag[i][LAB_RESULT_SELL_TIME], 'x').format('HH:mm:ss'),
								param: labFlag[i][LAB_RESULT_PARAMS]
							}
						)
					}
