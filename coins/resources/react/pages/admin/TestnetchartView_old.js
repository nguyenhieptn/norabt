...
class TestnetChartView extends Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			campaign: get(localStorage.getItem('default_campaign'), null),
	...
	render() {
		...
		for (let i in campaigns) {
			campaignOption[i] = campaigns[i][TESTNET_NAME];
		}
		...
		return (<>
			...
			<div style={{ position: 'relative', resize: 'both', overflow: 'auto' }} ref={c => this.chartContainer = c} onDoubleClick={() => {
				...
				<div style={{ display: 'flex', alignItems: 'center', position: 'absolute', top: 0, left: 0, zIndex: 100 }}>
					...
					<Input ref={c => this.campaignInput = c} placeholder="Campaign" className='input' struct={{
						[INPUT_TYPE]: 'select',
						[INPUT_ONCHANGE]: (e, obj) => {
							var id = obj.getValue();
							this.selectCampaign(id);
							localStorage.setItem('default_campaign', id);
	...
	selectCampaign(id) {
		if (!id) return;
		var campaigns = this.state.campaigns;
		App.symbol = campaigns[id][TESTNET_SYMBOL]
	...
	componentDidMount() {
		...
		var campaignModel = new Testnet_campaign();
		campaignModel.read(null).then(res => {
			if (res && res['result']) {
				var selected = null;
				var campaignIndex = {};
				if (res['data'].length > 0) {
					selected = localStorage.getItem('default_campaign');
					if(selected == null) selected = res['data'][0][TESTNET_ID];

					for (let i in res['data']) {
						campaignIndex[res['data'][i][TESTNET_ID]] = res['data'][i];
						if (res['data'][i][TESTNET_ID] == App.parsed['campaign']) {
							selected = res['data'][i][TESTNET_ID]
						}
					}
				}
	...
	getData(loading = true, limit = 1000, reset = true) {
		if (App.symbol == null || App.symbol == '' || this.state.campaign == null) return;
		if (loading) App.loading(true)
		return axios.request({
			url: `/admin/testnetchart/getCandleData`,
			method: 'POST',
			data: {
				symbol: App.symbol,
			...
			.then(response => {
				...
				if (response['result']) {
					...
					for (let i in flag) {
						...
						flagData.push(
							{
								x: get(flag[i][TESTNET_RESULT_MATCHED_TIME], flag[i][TESTNET_RESULT_TIME]),      // Point where the flag appears
								y: get(flag[i][TESTNET_RESULT_MATCHED], flag[i][TESTNET_RESULT_PRICE]),
								title: flag[i][TESTNET_RESULT_TYPE] == '1' ? 'Lab Long' : 'Lab Short', // Title of flag displayed on the chart 
								text: flag[i][TESTNET_RESULT_MATCHED],  // Text displayed when the flag are highlighted.
								fillColor: fillColor,
								profit: Number(flag[i][TESTNET_RESULT_PROFIT]),
								price: flag[i][TESTNET_RESULT_PRICE],
								time: moment(flag[i][TESTNET_RESULT_TIME], 'x').format('HH:mm:ss'),
								matched: flag[i][TESTNET_RESULT_MATCHED],
								matched_time: moment(flag[i][TESTNET_RESULT_MATCHED_TIME], 'x').format('HH:mm:ss'),
								sell: flag[i][TESTNET_RESULT_SELL],
...
export default TestnetChartView