...
class ChartView extends Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			histogram: get(localStorage.getItem('testnet_default_histogram'), '9'),
	...
	render() {
		...
		return (
			<>
				...
				<div style={{ width: '120%', height: (this.state.tooltip ? 85 : 0), border: 'solid thin darkgray' }}></div>
	...
	componentDidMount() {
		...
		if (App.accountSelectorTestnet) {
			App.accountSelectorTestnet.register('testnes_chart', () => {
				this.getAllData();
			});
		}
	...
	componentWillUnmount() {
		...
		if (App.accountSelectorTestnet) App.accountSelectorTestnet.unregister('testnes_chart')
	}
	...
	getData(loading = true, limit = 1000, reset = true) {
		...
		return axios.request({
			...
			data: {
				...
				interval: frame,
				limit: limit,
				account: App.accountSelectorTestnet.selected(),
			}
		})
			...
			.then(response => {
				...
				if (response['result']) {
					...
					var flag = get(allData['event_flag'], []);
					...
					if (!reset) {
						...
						for (let i in serials) {
							...
							else if (serial['id'] == 'RSI_WMA') rsi_wma = serial['data'].slice(0, serial['data'].length - 2);
						}
					...
					for (let i in flag) {
						var fillColor = "white";
						if (flag[i][TESTNET_RESULT_STATUS] == TESTNET_RESULT_STATUS_MATCHED) fillColor = 'orange';
						else if (flag[i][TESTNET_RESULT_STATUS] == TESTNET_RESULT_STATUS_TAKEPROFIT) fillColor = 'green';
						else if (flag[i][TESTNET_RESULT_STATUS] == TESTNET_RESULT_STATUS_STOPLOSS) fillColor = 'red';
						else if (flag[i][TESTNET_RESULT_STATUS] == TESTNET_RESULT_STATUS_CANCLE) fillColor = 'gray';
						flagData.push(
							{
								x: flag[i][TESTNET_RESULT_CHART],     // Point where the flag appears
								y: get(flag[i][TESTNET_RESULT_MATCHED_PRICE], flag[i][TESTNET_RESULT_ENTER_PRICE]),
								title: flag[i][TESTNET_RESULT_TYPE] == '1' ? 'Long' : 'Short', // Title of flag displayed on the chart 
								text: flag[i][TESTNET_RESULT_MATCHED_PRICE],  // Text displayed when the flag are highlighted.
								fillColor: fillColor,
								profit: Number(flag[i][TESTNET_RESULT_PROFIT]),
								price: flag[i][TESTNET_RESULT_ENTER_PRICE],
								time: moment(flag[i][TESTNET_RESULT_ENTER_TIME], 'x').format('HH:mm:ss'),
								matched: flag[i][TESTNET_RESULT_MATCHED_PRICE],
								matched_time: moment(flag[i][TESTNET_RESULT_MATCHED_TIME], 'x').format('HH:mm:ss'),
								sell: flag[i][TESTNET_RESULT_SELL_PRICE],
								sell_time: moment(flag[i][TESTNET_RESULT_SELL_TIME], 'x').format('HH:mm:ss'),
								param: flag[i][TESTNET_RESULT_PARAMS]
							}
						)
					}
					...
					for (let i in orderFlag) {
						...
						orderFlagData.push(
							{
								x: orderFlag[i][TESTNET_ORDER_TIME],      // Point where the orderFlag appears
								y: orderFlag[i][TESTNET_ORDER_PRICE],
								title: (orderFlag[i][TESTNET_ORDER_TYPE] == '1' ? 'Buy' : 'Sell') + " " + orderFlag[i][TESTNET_ORDER_PHASE], // Title of orderFlag displayed on the chart 
								text: orderFlag[i][TESTNET_ORDER_PRICE],  // Text displayed when the orderFlag are highlighted.
								fillColor: fillColor,

								price: orderFlag[i][TESTNET_ORDER_PRICE],
								time: moment(orderFlag[i][TESTNET_ORDER_TIME], 'x').format('HH:mm:ss'),
								qty: orderFlag[i][TESTNET_ORDER_QTY],
					...
					for (var i = 0; i < dataLength; i += 1) {
						...
						if (data[i]['rsi_wma'] != null) {
							rsi_wma.push([
								Number(data[i]['open_time']),
								Number(data[i]['rsi_wma'])
							])
						}
