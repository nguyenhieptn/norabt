...
import FuncAddAlertSystem from '../../components/admin/FuncAddAlertSystem';
import FuncEditAlertSystem from '../../components/admin/FuncEditAlertSystem';





class Alert_systemView extends Component {
    constructor(props) {
		super(props);
		this.id = makeId();
	...
	saveData() {
        ...
        var ctrl = new Ctrl();
        ctrl.set({ 'system_alter': JSON.stringify(this.state.config) }).then(res => {
            this.loadOrigin();
	...
	async loadOrigin() {
        var ctrl = new Ctrl();
        ctrl.get('system_alter', null).then(cfg => {
	...
	render() {
		...
		return (
			<>
				...
				<FuncAddAlertSystem reload={() => this.reloadOrigin()} ref={c => this.AddModel = c}></FuncAddAlertSystem>
				<FuncEditAlertSystem reload={() => this.reloadOrigin()} ref={c => this.EditModel = c}></FuncEditAlertSystem>
			</>
		)
	}
...
export default Alert_systemView;