...
class WatchlistView extends Component {
	...
	constructor(props) {
		...
		this.watchlist_struct[STRUCT_COLUMNS] = {
			...
			'busd_service': {
				[COL_NAME]: lang('BUSD Service'),
				[COL_DECORATOR_IN]: data => {
					return data ? <div style={{ color: '#00e209', fontWeight: 'bold', textAlign: 'center' }}>Running</div> : <div style={{ color: 'red', fontWeight: 'bold', textAlign: 'center' }}>Stopped</div>
				}
			},
			'orderbook_service': {
				[COL_NAME]: lang('OrderBook Service'),
				[COL_DECORATOR_IN]: data => {
					return data ? <div style={{ color: '#00e209', fontWeight: 'bold', textAlign: 'center' }}>Running</div> : <div style={{ color: 'red', fontWeight: 'bold', textAlign: 'center' }}>Stopped</div>
				}
			},
			[WL_IS_KLINE]: {
				[COL_NAME]: lang(WL_IS_KLINE),
				// [COL_DECORATOR_IN]: data => {
				// 	return data ? <div style={{ color: '#00e209', fontWeight: 'bold', textAlign: 'center' }}>Running</div> : <div style={{ color: 'red', fontWeight: 'bold', textAlign: 'center' }}>Stopped</div>
				// }
			},
			[WL_IS_BUSD]: {
				[COL_NAME]: lang(WL_IS_BUSD),
				// [COL_DECORATOR_IN]: data => {
				// 	return data ? <div style={{ color: '#00e209', fontWeight: 'bold', textAlign: 'center' }}>Running</div> : <div style={{ color: 'red', fontWeight: 'bold', textAlign: 'center' }}>Stopped</div>
				// }
			},
				...
				<Table ref={c => this.table = c} table={this.watchlist_struct} autoload={true}>
					<FuncBar
						left={<>
							...
							<div className='button btn btn-primary btn-sm' onClick={() => { this.startCrawlerBUSD() }} style={{ fontSize: 12, marginRight: 0 }}>Start BUSD</div>
							<div className='button btn btn-danger btn-sm' onClick={() => { this.stopCrawlerBUSD() }} style={{ fontSize: 12, marginRight: 0 }}>Stop BUSD</div>
							<div className='button btn btn-primary btn-sm' onClick={() => { this.startCrawlerOrderBook() }} style={{ fontSize: 12, marginRight: 0 }}>Start OrderBook</div>
							<div className='button btn btn-danger btn-sm' onClick={() => {this.stopCrawlerOrderBook() }} style={{ fontSize: 12, marginRight: 0 }}>Stop OrderBook</div>
						</>}
	...
	async startCrawlerBUSD() {
		...
		var confirm = await makeQuestion('Do you want to restart crawl Data: ' + wl.join(', ') + '?');
		if (!confirm) return;
		...
		for (let i in wl) {
			...
			let result = await axios.request({
				url: `/admin/watchlist/restartBusdService`,
				method: 'POST',
				data: {
					symbol: wl[i]
				}
	...
	async stopCrawlerBUSD() {
		...
		for (let i in wl) {
			App.loading(true)
			axios.request({
				url: `/admin/watchlist/stopBusdService`,
				method: 'POST',
				data: {
					symbol: wl[i]
				}
				...
				.then(response => {
					...
					if (response['result']) {
						...
						this.filterTimeout = setTimeout(()=>this.table.filter(), 500);
					}
					else {
						error_handle(response);
					}
	...
	async startCrawlerOrderBook() {
		...
		for (let i in wl) {
			...
			let result = await axios.request({
				url: `/admin/watchlist/restartOrderBookService`,
				method: 'POST',
				data: {
					symbol: wl[i]
				}
	...
	async stopCrawlerOrderBook() {
		...
		for (let i in wl) {
			App.loading(true)
			axios.request({
				url: `/admin/watchlist/stopOrderBookService`,
				method: 'POST',
				data: {
					symbol: wl[i]
				}
