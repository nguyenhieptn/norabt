...
import FuncAddAlertStock from '../../components/admin/FuncAddAlertStock';
import FuncEditAlertStock from '../../components/admin/FuncEditAlertStock';




class Alert_stockView extends Component {
    constructor(props) {
		super(props);
		this.id = makeId();
	...
	async loadOrigin() {
        var ctrl = new Ctrl();
        ctrl.get('stock_alter', null).then(cfg => {
	...
	render() {
		...
		return (
			<>
				...
				<FuncAddAlertStock reload={() => this.reloadOrigin()} ref={c => this.AddModel = c}></FuncAddAlertStock>
				<FuncEditAlertStock reload={() => this.reloadOrigin()} ref={c => this.EditModel = c}></FuncEditAlertStock>
			</>
		)
	}
...
export default Alert_stockView;