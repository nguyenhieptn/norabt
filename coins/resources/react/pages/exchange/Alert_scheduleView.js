...
import FuncAddAlertSchedule from '../../components/admin/FuncAddAlertSchedule';
import FuncEditAlertSchedule from '../../components/admin/FuncEditAlertSchedule';


class Alert_scheduleView extends Component {
    constructor(props) {
		super(props);
		this.id = makeId();
	...
	saveData() {
        var ctrl = new Ctrl();
        ctrl.set({ 'alert_schedule': JSON.stringify(this.state.config) }).then(res => {
			this.loadOrigin();
	...
	async loadOrigin() {
		var ctrl = new Ctrl();
		ctrl.get('alert_schedule', null).then(cfg => {
	...
	render() {
		...
		return (
			<>
                ...
                <FuncAddAlertSchedule reload={() => this.reloadOrigin()} ref={c => this.AddModel = c}></FuncAddAlertSchedule>
                <FuncEditAlertSchedule reload={() => this.reloadOrigin()} ref={c => this.EditModel = c}></FuncEditAlertSchedule>
			</>
		)
	}
}

export default Alert_scheduleView;