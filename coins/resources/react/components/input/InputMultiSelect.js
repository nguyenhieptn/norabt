...
class InputMultiSelect extends Component {
	...
	drawOptions(){
		var optionHtml = [];
		for(let i in this.options){
			var item = <div key={i} className="dropdown-item"><label style={{display:'flex', textAlign:'center'}}>
				<input checked={isset(this.state.value[i]) && this.state.value[i]} onChange={(event)=>{this.updateValue(i, event.target.checked)}} type="checkbox"/>&nbsp;{this.options[i]}</label>
			</div>
		
			if(this.options[i] == 'All'){
				optionHtml.unshift(item) ;
			}else{
				optionHtml.push(item) ;
			}
