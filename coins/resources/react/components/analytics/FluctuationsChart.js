...
class FluctuationsChart extends Component {
    ...
    async getData(symbol, frame) {
        ...
        datas = datas.sort((a, b) => Number(a[colNames['time']]) - Number(b[colNames['time']]));
        ...
        for (let i in datas) {
            if (isset(datas[i - 1])) {
                ...
                var time = Number(datas[i][colNames.opentime]);
                if (time == 0) continue;
                ...
                if (Number(datas[i - 1][colNames.high]) > 0) {
                    ...
                    if (this.state.minRSI4h1 != '' || this.state.maxRSI4h1 != '' || this.state.minRSI4h0 != '' || this.state.maxRSI4h0 != '') {
                        ...
                        if (!isset(data4hIndex[openTime4h])) {
                            ...
                            if (!isset(data4hIndex[openTime4h - 14400000])) {
                                ...
                                var rsi4h1 = data4hIndex[openTime4h - 14400000][CANDLE_4H_RSI14]
                            }
                    ...
                    if (this.state.minRSI1h0 != '' || this.state.maxRSI1h0 != '') {
                        ...
                        if (!isset(data1hIndex[openTime1h])) {
                            ...
                            var rsi1h0 = data1hIndex[openTime1h][CANDLE_1H_RSI14]
                        }
    ...
    async loadData(symbol, frame) {
        ...
        var symbolData = await model.get([[[colNames.symbol, '=', symbol]]], { orderBy: colNames.time, asc: false, limit: 500 });
