...
class ChartFlowRisky4 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            selected: true,
            ...
            ChartOptions: {
                ...
                title: {
                    text: '<b> Flow Risky4</b>',
                    // align: 'left',
                    // x: 60,
                    // y: 18
                },
                xAxis: {
                    categories: ['4h-Risky4', '1d-Risky4'],
                },
                yAxis: {
        ...
        resultModel.getByAccountTestnet(App.accountSelectorTestnet.selected()).then(res => {
            if (res['data']) {
                ...
                res.map(item => {
                    ...
                    var symbol = item[TESTNET_RESULT_SYMBOL];
                    var flow = item[TESTNET_RESULT_FLOW]

                    var index = datasymbol.indexOf(symbol);
                    if (index > -1) {
                        if (flow == '1D-Risky4-LONG') {

                            data1D[index] += 1;
                        } else if (flow == '4H-Risky4-LONG') {
                            data4H[index] += 1;
                        }
...
export default ChartFlowRisky4;