		...
		return (
				...
				<style>{`
				 ...
				 <div style={{display: 'flex', position:'relative'}}>
			 		...
			 		<i style={{position: 'relative', fontSize: 20}} className="button fa fa-bell-o" onClick={()=>{this.setState({expand: !this.state.expand})}}>
			 			<label id="number_new_logs">{this.state.new_logs}</label>
			 		</i>
			 		{(this.state.expand && this.state.logs.length > 0)?
			 		<>
				 		
				 		<div className="syslog_frame">
		 						...
		 						return (
		 								<div key={i} className="syslog_item" onClick={()=>{this.readLog(i)}}>
		 									<div className="syslog_head" style={{color:this.levelColor[item[NOTICE_LEVEL]]}}>
		 										...
		 										<span>{moment(item[NOTICE_TIME], 'X').format(DATE_FORMAT)}</span>&nbsp;&nbsp;
		 									</div>
		 									 
		 									<div className="syslog_content"><Link to={get(item[NOTICE_ACTION], '#')}>
							...
							<div>
								<div className="button syslog_readall" onClick={()=>{this.readAllLog()}}>{lang('Đánh dấu đã đọc')}</div>
								<div className="button syslog_readmore" onClick={()=>{this.readMore(10)}}>{lang('Xem thêm')}</div>
							</div>		
				 		</div>
			 		</>:''}
