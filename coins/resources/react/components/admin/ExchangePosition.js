...
import OrderBinanceModal from './OrderBinanceModal'
...
class ExchangePosition extends Component {
	...
	constructor(props) {
		...
		this.actions_struct[STRUCT_COLUMNS] = {
			...
			[ACTION_SYMBOL]: {
				...
				[COL_DECORATOR_IN]: (data) => {
					...
					return <div style={{ cursor: 'pointer' }} onClick={() => {
						this.props.onSelectSym && this.props.onSelectSym(data);
					}}><div><b>
			...
			// 'TimeOrder': {
			// 	[COL_NAME]: 'Time Order(m)',
			// 	[COL_STYLE]: { textAlign: 'center' },
			// 	[COL_DECORATOR_IN]: (colId, rowId, data) => {

			// 		// var sell_time =  moment(data[rowId][ACTION_SELL_TIME] , 'x');
			// 		// var enter_time =  moment(data[rowId][ACTION_CHART] , 'x');
			// 		// if(isNaN(sell_time) || isNaN(enter_time)) return;

			// 		// var diff = sell_time.diff(enter_time , 'minutes');
			// 		// if(diff > 60)  	return <b style={{color : 'red'}} >{diff }</b>;


			// 		// return <b  >{diff }</b>;

			// 		var interval = data[rowId][ACTION_SELL_TIME];
			// 		var resultChart = data[rowId][ACTION_CHART];
			// 		var result;
			// 		if (!interval) {
			// 			result = Number(moment().format('x')) - Number(resultChart);
			// 		} else {
			// 			result = Number(interval) - Number(resultChart);
			// 		}
			...
			'inveted': {
				...
				[COL_DECORATOR_IN]: (colId, rowId, data) => {
					...
					return formatNumber((matched * quantity).toFixed(3)) + "";
			...
			[ACTION_BUDGET]: {
				...
				[COL_SORT]: true,
				[COL_STYLE]: { textAlign: 'center' },
				[COL_DECORATOR_IN]: (data) => (data != null && data != "") ? formatNumber(data.toFixed(3)) + "" : "",
			...
			base_profit_margin: {
				...
				[COL_DECORATOR_IN]: (colId, rowId, data) => {
					...
					return formatNumber((matchedQty * baseProfit * matchedPrice / 100).toFixed(3)) + " (" + (targetProfit).toFixed(3) + "%)";
				}
			},
			...
			price: {
				...
				[COL_DECORATOR_IN]: (colId, rowId, data) => {
					...
					var style = { fontWeight: 'bold' };
					return <div style={style}>{(price).toFixed(5)}</div>;
				}
			},
			[ACTION_MATCHED_PRICE]: {
				[COL_NAME]: lang(ACTION_MATCHED_PRICE),
				[COL_SORT]: true,
				[COL_DECORATOR_IN]: data => Number(data).toFixed(4),
				[COL_STYLE]: { textAlign: 'center', fontWeight: 'bold' },
			...
			[ACTION_LIQUIDATION]: {
				...
				[COL_SORT]: true,
				[COL_DECORATOR_IN]: data => <div style={{ color: 'red' }}>{Number(data).toFixed(6)}</div>,
				[COL_STYLE]: { textAlign: 'center', fontWeight: 'bold' },
			},
			...
			[ACTION_BASEPROFIT]: {
				...
				[COL_SORT]: true,
				[COL_STYLE]: { textAlign: 'center', fontWeight: 'bold' },
				[COL_DECORATOR_IN]: (data) => (data != null && data != "") ? <div style={{ color: 'blue' }}>{data.toFixed(3) + "%"}</div> : ""
		...
		this.actions_struct[STRUCT_ROWS] = {
			...
			[ROW_FOOTER]: (datas, visibleColumns) => {
				...
				return <tr>
					...
					<td colSpan={4} style={{ fontWeight: 'bold', color: (totalRealProfit > 0 ? "green" : "red"), textAlign: 'right' }}>Real Profit: {totalRealProfit.toFixed(3)}%</td>
					<td colSpan={1} style={{ fontWeight: 'bold', color: (totalRealProfit > 0 ? "green" : "red"), textAlign: 'right' }}>Total Profit: {totalProfit.toFixed(3)}%</td>

					<td colSpan={1} style={{ fontWeight: 'bold', color: (totalRealProfit > 0 ? "green" : "red"), textAlign: 'right' }}>Money: {(totalPNL - totalCommit).toFixed(3)} USDT</td>
					<td colSpan={1} style={{ fontWeight: 'bold', color: (totalRealProfit > 0 ? "green" : "red"), textAlign: 'right' }}>Total PNL: {totalPNL.toFixed(3)} USDT</td>
					<td colSpan={1}></td>
					<td colSpan={1} style={{ fontWeight: 'bold', color: (totalRealProfit > 0 ? "green" : "red"), textAlign: 'right' }}>Commit: {totalCommit.toFixed(3)} USDT</td>
		...
		// this.actions_struct[STRUCT_CELLS] = {};
		// this.actions_struct[STRUCT_CELLS][CELL_STYLE] = (colID, rowID, tableData)=>{
		// 	var rowData = tableData[rowID];
		// 	var data = rowData[colID];
		// 	if([ACTION_MARGIN, ACTION_PENDING, ACTION_PHASE].includes(colID)) return {};
		// 	var style = {};
		// 	if(data >= 0) style['color'] = 'limegreen';
		// 	if(data < 0) style['color'] = 'red';
		// 	return style;
			...
			<div >
				<Table ref={c => this.table = c} table={this.actions_struct} autoload={false}>
					<FuncBar
						...
						right={<><FuncAdd /><FuncDel /><FuncEdit /><FuncClear /><FuncRefresh /><FuncExport /></>}></FuncBar>
					<MainTable className='table table-bordered table-striped table-resizable' minHeight={350}></MainTable>
					<Pagination></Pagination>


				</Table>

				<OrderModal ref={c => this.orderModal = c} onSelectTime={(sym, start, stop) => { this.props.onSelectTime && this.props.onSelectTime(sym, start, stop) }}></OrderModal>
				<OrderBinanceModal onSelectTime={(sym, start, stop) => { this.props.onSelectTime && this.props.onSelectTime(sym, start, stop) }} ref={c => this.OrderBinanceModal = c}></OrderBinanceModal>
