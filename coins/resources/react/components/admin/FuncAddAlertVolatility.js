...
class FuncAddAlertVolatility extends Component {
    constructor(props) {
        super(props);
        this.id = makeId();
        this.state = {
            ...
            columns: {
                1: '1d_high_low_avg3d',
                2: '1d_high_high_avg3d',
                3: '1d_low_low_avg3d',
                4: '4h_high_low_avg3d',
                5: '4h_high_high_avg3d',
                6: '4h_low_low_avg3d',
                7: '1d_high_low_avg7d',
                8: '1d_high_high_avg7d',
                9: '1d_low_low_avg7d',
                10: '4h_high_low_avg7d',
                11: '4h_high_high_avg7d',
                12: '4h_low_low_avg7d',
            },
    ...
    saveData() {
        ...
        var ctrl = new Ctrl();
        ctrl.set({ 'track_avg_alter': JSON.stringify(data) }).then(res => {
            this.props.reload();
                                        ...
                                        {conditions.map((condition, conIndex) => {
                                            ...
                                            return <div key={condition.id} className='box_flex box_border' style={{ padding: 5, margin: 5, borderRadius: 5, background: 'white' }}>
                                                ...
                                                <div style={{ flex: 5, margin: 5 }}>
                                                    <select className='input' style={{ width: '100%' }} value={condition.volatility} onChange={e => {
                                                        condition.volatility = e.target.value;
                                                        this.setState({ config });
                                                    }}>
                                                        {Object.keys(this.state.volatility).map(col => {
                                                            return <option key={col} value={col}>{lang(this.state.volatility[col])}</option>
                                                        })}
                                                    </select>
                                                </div>

                                                <div style={{ flex: 5, margin: 5 }}>
                                                    <select className='input' style={{ width: '100%' }} value={condition.frame} onChange={e => {
                                                        condition.frame = e.target.value;
                                                        this.setState({ config });
                                                    }}>
                                                        {Object.keys(this.state.frame).map(col => {
                                                            return <option key={col} value={col}>{this.state.frame[col]}</option>
                                                        })}
                                                    </select>
                                                </div>

                                                <div style={{ flex: 5, margin: 5 }}>
                                                    <select className='input' style={{ width: '100%' }} value={condition.avg} onChange={e => {
                                                        condition.avg = e.target.value;
                                                        this.setState({ config });
                                                    }}>
                                                        {Object.keys(this.state.avg).map(col => {
                                                            return <option key={col} value={col}>{this.state.avg[col]}</option>
                                                        })}
                                                    </select>
                                                </div>
...
export default FuncAddAlertVolatility;