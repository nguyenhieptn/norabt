...
class Pagination extends Component {
	...
	editNumberPage(event) {
		...
		localStorage.setItem("page_quantity_" + this.table[STRUCT_TABLE][DATA_TABLE_ID], value);
		this.table.filter();
	}
			...
			<>
				...
				<div style={{ display: 'flex' }}>
					...
					<span className="page_break" style={{ margin: 'auto auto auto 0px', display: 'flex', alignItems: 'center' }}>
						...
						<span className="step step_pre" onClick={() => this.editCurrentPage('down', 1)}>{lang('Previous')}</span>
						...
						<div style={{ fontWeight: 'bold' }}>{lang("Total") + ": " + this.table[STRUCT_TABLE][PAGE_TOTAL]}</div>
					</span>
