...
if( $sp = websocket_open('ssl://realtime-iboard.ssi.com.vn:443','',$errstr, 10, '/graphql') ) {
  echo "Sending message to server: '$message' \n";
  websocket_write($sp, '{"type":"connection_init","payload":{}}');
  echo "sever send 1 " . websocket_read($sp) . "\n" ;
  websocket_write($sp, '{"id":"1","type":"start","payload":{"variables":{},"extensions":{},"operationName":null,"query":"subscription {\n  systemStatusChanged {\n    status\n    __typename\n  }\n}\n"}}');
  //echo "sever send 2" . websocket_read($sp) . "\n";
  websocket_write($sp, '{"id":"2","type":"start","payload":{"variables":{},"extensions":{},"operationName":"serverName","query":"subscription serverName {\n  serverName\n}\n"}}');
  //echo "sever send 3" . websocket_read($sp) . "\n";
  //websocket_write($sp, '{"id":"3","type":"start","payload":{"variables":{"markets":["hose","hnx","upcom"]},"extensions":{},"operationName":"notifySessionByList","query":"subscription notifySessionByList($markets: [String!]!) {\n  notifySessionByList(markets: $markets) {\n    market\n    session\n    __typename\n  }\n}\n"}}');
  //echo "sever send 4" . websocket_read($sp) . "\n";
  //websocket_write($sp, '{"id":"4","type":"start","payload":{"variables":{"indexIds":"VNINDEX,VN30,HNX30,HNXIndex,HNXUpcomIndex,VNXALL"},"extensions":{},"operationName":"notifyIndexRealtimeBySelection","query":"subscription notifyIndexRealtimeBySelection($indexIds: String!) {\n  notifyIndexRealtimeBySelection(indexIds: $indexIds) {\n    indexID\n    indexValue\n    allQty\n    allValue\n    totalQtty\n    totalValue\n    advances\n    declines\n    nochanges\n    ceiling\n    floor\n    totalQttyPrev\n    time\n    __typename\n  }\n}\n"}}');
  
  websocket_write($sp, '{"id":"5","type":"start","payload":{"variables":{"arr":"hnx:8704"},"extensions":{},"operationName":"stockRealtimeBySelection","query":"subscription stockRealtimeBySelection($arr: String!) {\n  stockRealtimeBySelection(arr: $arr) {\n    data\n    __typename\n  }\n}\n"}}');
  while(true){
    echo websocket_read($sp);
	echo "\n";
  }
}else {
...
\*============================================================================*/
function websocket_open($address, $headers='',&$error_string='',$timeout=10, $path = '/', $context = null){
