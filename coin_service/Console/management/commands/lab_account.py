...
from Console.Phoenix.Lab.AccountAsyncImp import AccountAsyncImp
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # try:
        account = LabAccountWrapper().get({LabAccountWrapper.lab_account_id: accountId}) #type: LabAccount
            
        startPoint = time()
        if(account.lab_account_sync == 0):
            accountImp = AccountAsyncImp(accountId)
            accountImp.initial()
            print(accountImp.start())
        else:
