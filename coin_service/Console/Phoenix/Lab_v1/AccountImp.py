...
from Console.Models.Wrappers.Lab.Coin_lab import (
    LabAccountWrapper,
    LabCampaignsWrapper,
    LabEventLogsWrapper,
    LabResultsWrapper,
    LabTrackBalanceWrapper,
)
...
class AccountImp(AccountInterface):
    ...
    def initial(self, rawRun=False):
        ...
        if not self.account:
            raise Exception("Can not foud any account")
        ...
        # create models
        if (
            self.account.lab_account_db is None
            or self.account.lab_account_db == ""
        ):
            self.account.lab_account_db = "backtest_data"
        ...
        print(
            f"Start Lab in Sync mode. Block Data Leng {self.blockDataLeng} days"
        )
        ...
        self.campaigns = (
            LabCampaignsWrapper()
            .filter(lab_campaign_account=self.account.lab_account_id)
            .order_by("-" + LabCampaignsWrapper.lab_campaign_priority)
        )
        ...
        for campaign in self.campaigns:
            ...
            if self.startTime is None:
                ...
                if tempStartTime < self.startTime:
                    self.startTime = tempStartTime

            if self.stopTime is None:
                self.stopTime = tempStopTime
            else:
                if tempStopTime < self.stopTime:
                    self.stopTime = tempStopTime
        ...
        for campaign in self.campaigns:
            ...
            if isset(self.context, contextKey):
                ...
                campaignTotal = self.klineModel.setCollection(
                    "candle_1m"
                ).count(
                    {
                        "symbol": symbol,
                        "timestamp": {
                            "$gte": self.startTime / 1000,
                            "$lte": self.stopTime / 1000,
                        },
                    }
                )
    ...
    def createCampaignImps(self, strategyId=None, rawRun=False):
        ...
        for campaign in self.campaigns:
            ...
            campImp = (
                CampaignImp1m(campaign, self)
                if self.is1m
                else CampaignImp(campaign, self)
            )
            campImp.initial(self.optiParams, strategyId, rawRun)

            # save strategy for optimization result
            if not isset(
                self.strategies, campImp.campaign.lab_campaign_strategy
            ):
                self.strategies[campImp.campaign.lab_campaign_strategy] = (
                    campImp.cfgStrategy
                )
                ...
                if not checkMatchPriceResult["result"]:
                    ms = (
                        Tele.TELE_ICON_WARNING
                        + " LAB ["
                        + self.account.lab_account_name
                        + "] "
                        + checkMatchPriceResult["message"]
                    )
                    Tele.send(
                        ms, Tele.TELE_SIMULATE_ERROR, Tele.TELE_BOT_DEFAULT
                    )
                scanLeng = scanDataLeng(campImp.cfgStrategy)
                if self.checkDataLeng < scanLeng:
                    self.checkDataLeng = scanLeng
    ...
    def createCheckData(self):
        ...
        if isset(self.context, "createCheckDataResult"):
            self.checkData = copy.deepcopy(
                self.context["createCheckDataResult"]
            )
        else:
            ...
            for symbol in self.totalNeedData:
                ...
                if isset(self.totalNeedData[symbol], "kline"):
                    ...
                    for frame in self.totalNeedData[symbol]["kline"]:
                        ...
                        firstData = (
                            self.klineModel.setCollection(collName)
                            .find(
                                {
                                    "symbol": symbol,
                                    "timestamp": {
                                        "$lte": int(self.startTime / 1000)
                                    },
                                }
                            )
                        ...
                        if not isset(firstData, 0):
                            continue
                        firstData = firstData[0]
                        self.checkData[symbol]["kline"][frame].insert(
                            0, firstData
                        )
                        nextData = (
                            self.klineModel.setCollection(collName)
                            ...
                            .sort("timestamp", DESCENDING)
                            .limit(
                                -self.totalNeedData[symbol]["kline"][frame] + 1
                            )
                        )
                ...
                if isset(self.totalNeedData[symbol], "pressure"):
                    pressureData = (
                        ...
                        .sort("timestamp", DESCENDING)
                        .limit(-self.totalNeedData[symbol]["pressure"] + 1)
                    )
                    self.checkData[symbol]["pressure"] = list(pressureData)

            self.context["createCheckDataResult"] = copy.deepcopy(
                self.checkData
            )
    ...
    def getData(self, startTime, stopTime):
        ...
        for symbol in self.totalNeedData:
            ...
            if isset(self.totalNeedData[symbol], "kline"):
                ...
                for frame in self.totalNeedData[symbol]["kline"]:
                    ...
                    if len(klineFrameData) > 0:
                        klineFramePandas = (
                            pd.DataFrame.from_dict(klineFrameData)
                            .drop_duplicates(subset="timestamp", keep="last")
                            .set_index("timestamp", drop=False)
                            .drop(columns=["_id"])
                        )
                        indexData[symbol]["kline"][frame] = (
                            klineFramePandas.to_dict("index")
                        )
            ...
            if isset(self.totalNeedData[symbol], "busd"):
                ...
                if len(busdData) > 0:
                    busdPandas = (
                        pd.DataFrame.from_dict(busdData)
                        .drop_duplicates(subset="timestamp", keep="last")
                        .set_index("timestamp", drop=False)
                        .drop(columns=["_id"])
                    )
            ...
            if isset(self.totalNeedData[symbol], "orderbook"):
                ...
                if len(orderbookData) > 0:
                    orderbookPandas = (
                        pd.DataFrame.from_dict(orderbookData)
                        .drop_duplicates(subset="timestamp", keep="last")
                        .set_index("timestamp", drop=False)
                    )
                    indexData[symbol]["orderbook"] = orderbookPandas.to_dict(
                        "index"
                    )
            ...
            if isset(self.totalNeedData[symbol], "pressure"):
                ...
                if len(pressureData) > 0:
                    pressurePandas = (
                        pd.DataFrame.from_dict(pressureData)
                        .drop_duplicates(subset="timestamp", keep="last")
                        .set_index("timestamp", drop=False)
                    )
                    indexData[symbol]["pressure"] = pressurePandas.to_dict(
                        "index"
                    )
        # print(indexData)
        return indexData
    ...
    def startStatus(self):
        ...
        LabCampaignsWrapper().filter(
            lab_campaign_account=self.account.lab_account_id
        ).update(
        ...
        LabTrackBalanceWrapper().drop(
            lab_track_bl_account=self.account.lab_account_id
        )
        LabResultsWrapper().drop(lab_result_account=self.account.lab_account_id)
        for campaign in self.campaigns:
            LabEventLogsWrapper().drop(
                lab_elog_campaign=campaign.lab_campaign_id
            )
    ...
    def run(
        self, optiParams={}, context={}, lock=None, optiId=0, blockDataLeng=None
    ):
    ...
    def runMainStrategy(self):
        ...
        if getDataStopTime > self.stopTime:
            getDataStopTime = self.stopTime
        ...
        choicePeriodTime = (
            Number(self.account.lab_account_choice_period) * 24 * 60 * 60000
        )
        ...
        while True:
            ...
            if os.path.isfile(tempFile):
                ...
                print(
                    "Query Database from "
                    + str(
                        datetime.fromtimestamp(floor(getDataStartTime / 1000))
                    )
                    + " to "
                    + str(datetime.fromtimestamp(floor(getDataStopTime / 1000)))
                )
                indexData = self.getData(
                    int(getDataStartTime / 1000) + 1,
                    int(getDataStopTime / 1000),
                )
            ...
            for symbol in indexData:
                if (
                    symbol in self.totalDependSymbol
                    and symbol not in self.totalNeedData
                ):
                    continue
                if "kline" not in indexData[symbol]:
                    continue
                for frame in indexData[symbol]["kline"]:
                    if len(indexData[symbol]["kline"][frame]) > len(
                        longSymData
                    ):
                        longSymData = indexData[symbol]["kline"][frame]
            ...
            if len(longSymData) == 0:
                ...
                getDataStopTime = (
                    getDataStartTime + self.blockDataLeng * 86400 * 1000
                )
                if getDataStopTime > self.stopTime:
                    getDataStopTime = self.stopTime
                continue
            ...
            for runtime in longSymData:
                ...
                for symbol in self.totalNeedData:
                    ...
                    if isset(needDataSym, "kline"):
                        ...
                        for frame in needDataSym["kline"]:
                            if isset(indexData, symbol) and isset(
                                indexData[symbol]["kline"][frame], runtime
                            ):
                                lastData["kline"][frame] = indexData[symbol][
                                    "kline"
                                ][frame][runtime]

                    if isset(needDataSym, "busd"):
                        if isset(indexData, symbol) and isset(
                            indexData[symbol]["busd"], runtime
                        ):
                            lastData["busd"] = indexData[symbol]["busd"][
                                runtime
                            ]

                    if isset(needDataSym, "orderbook"):
                        if isset(indexData, symbol) and isset(
                            indexData[symbol]["orderbook"], runtime
                        ):
                            lastData["orderbook"] = indexData[symbol][
                                "orderbook"
                            ][runtime]

                    if isset(needDataSym, "pressure"):
                        if isset(indexData, symbol) and isset(
                            indexData[symbol]["pressure"], runtime
                        ):
                            lastData["pressure"] = indexData[symbol][
                                "pressure"
                            ][runtime]

                    if len(lastData.keys()) > 0:
                        updateCheckData(
                            self.checkData, symbol, lastData, self.checkDataLeng
                        )
                ...
                for campaign in self.campaigns:
                    ...
                    if not isset(activeSymbol, campSym):
                        continue
                    proce = self.campaignImps[
                        campaign.lab_campaign_id
                    ]  # type: CampaignImp
                    ...
                    if self.choiceCoinData is not None:
                        choiceTime = (
                            floor(runtimeInt / choicePeriodTime)
                            * choicePeriodTime
                        )
                        if isset(self.choiceCoinData, choiceTime):
                            allowSymbol = self.choiceCoinData[choiceTime][
                                "symbols"
                            ]
                            if campSym not in allowSymbol:
                                continue

                    proce = self.campaignImpsBySymbol[
                        campSym
                    ]  # type: CampaignImp
                ...
                if processed % 2000 == 0:
                    ...
                    for campaign in self.campaigns:
                        campaign.lab_campaign_running = 1
                        campaign.lab_campaign_status = (
                            str(self.total[self.longSym]) + "," + str(processed)
                        )
                        campaign.lab_campaign_runtime = runtimeInt

                    if not self.isOptimize:
                        LabCampaignsWrapper().filter(
                            lab_campaign_id__in=keys
                        ).update(
                            lab_campaign_running=1,
                            lab_campaign_status=str(self.total[self.longSym])
                            + ","
                            + str(processed),
                    ...
                    print(
                        "Lab Speed : " + str(speed) + " events/second", end="\r"
                    )
                    speedTime = time()
        ...
        for campaign in self.campaigns:
            ...
            campaign.lab_campaign_status = (
                str(processed) + "," + str(self.total[self.longSym])
            )
        ...
        for campaignId in self.campaignImps:
            ...
            if not campEvent is None and campEvent.lab_result_pending == 1:
                ...
                intervalTime = Number(campImp.frame1m["event_time"]) - Number(
                    campEvent.lab_result_chart
                )
                campEvent.lab_result_interval = intervalTime

                package = Number(campEvent.lab_result_budget)
                totalProfit = Number(campEvent.lab_result_eventprofit) + Number(
                    campEvent.lab_result_profit
                )
                invest = Number(campEvent.lab_result_matched_qty) * Number(
                    campEvent.lab_result_matched_price
                )

                campEvent.lab_result_realpnl = invest * totalProfit / 100
                if package > 0:
                    campEvent.lab_result_realprofit = (
                        campEvent.lab_result_realpnl * 100 / package
                    )

                if campImp.maxInterval < intervalTime:
                    campImp.maxInterval = intervalTime
                campImp.totalInterval += intervalTime
                campImp.totalPosition += 1
                self.pendingEvent.append(campEvent)

            if self.maxInterval < campImp.maxInterval:
                self.maxInterval = campImp.maxInterval
            self.totalPosition += campImp.totalPosition
            self.totalInterval += campImp.totalInterval
        if self.totalPosition > 0:
            self.avgInterval = self.totalInterval / self.totalPosition
        ...
        if not self.isOptimize:
            ...
            mss = (
                ...
                + "] Finished"
                + "\n- Origin Balance: "
                + str(Round(self.originBalance, 3))
                + " USDT"
                + "\n- Balance: "
                + str(Round(self.balance, 3))
                + " USDT"
                + "\n- Margin Balance: "
                + str(Round(self.marginBalance, 3))
                + " USDT"
                + "\n- Close Position: "
                + str(self.totalPosition)
                + "\n- Take Profit: "
                + str(self.totalTakeprofit)
                + "\n- Stoploss: "
                + str(self.totalStoploss)
                + "\n- Pending Position: "
                + str(len(self.pendingEvent))
                + "\n- Max Unrealize: "
                + str(Round(-self.maxUnrealize, 3))
                + " %"
                + "\n- Max Invest: "
                + str(Round(self.maxInvest, 3))
                + " %"
                + "\n- Max Interval: "
                + str(Round(self.maxInterval / 60000))
                + " minutes"
                + "\n- Avg Interval: "
                + str(Round(self.avgInterval / 60000))
                + " minutes"
            )
            try:
                ...
                account = {
                    "origin_balance": self.originBalance,
                    "balance": self.balance,
                    "margin_balance": self.marginBalance,
                    ...
                    "avg_interval": Round(self.avgInterval / 60000),
                }

                collection.update_one(
                    {"account_name": self.account.lab_account_name},
                    {"$set": account},
                    upsert=True,
                )
            except Exception as e:
                print(f"❌ Lỗi khi lưu vào MongoDB: {e}")
            Tele.send(mss, Tele.TELE_SIMULATE, Tele.TELE_BOT_DEFAULT)

    __trackBalanceData = []

    def checkLiquidation(self, events, runtime):
        if (
            self.account.lab_account_margin_type == "CROSS"
            or self.account.lab_account_track_balance == 1
        ):
            totalUnrelize = 0
            totalInvest = 0
            totalBudget = 0
            ...
            for campImp in events:
                ...
                if (
                    campImp.event.lab_result_type
                    == LabResultsWrapper.LAB_RESULT_TYPE_LONG
                ):
                    worstPrice = low
            ...
            if percentInvest > self.maxInvest:
                self.maxInvest = percentInvest
            if percentUnrealize > self.maxUnrealize:
                self.maxUnrealize = percentUnrealize
            self.marginBalance = balance - totalUnrelize

            updatedAccount = self.account
            if not self.isOptimize:
                if (
                    self.account.lab_account_track_balance == 1
                    and totalInvest > 0
                ):

                    if len(self.__trackBalanceData) > 1000:
                        LabTrackBalanceWrapper().bulk_create(
                            self.__trackBalanceData
                        )
                        self.__trackBalanceData = []
                    else:
                        self.__trackBalanceData.append(
                            LabTrackBalance(
                                **{
                                    ...
                                    LabTrackBalanceWrapper.lab_track_bl_margin_bl: balance
                                    - totalUnrelize,
    ...
    def countPosition(self):
        ...
        for campaignId in self.campaignImps:
            ...
            if event is None:
                continue
            if event.lab_result_pending == 0:
                continue
            if (
                event.lab_result_matched_qty == 0
                and event.lab_result_status
                == LabResultsWrapper.LAB_RESULT_STATUS_ENTER_WAITTING
            ):
                continue
            count += 1
        return count
    ...
    def countSlot(self, strategyId):
        ...
        for campaignId in self.campaignImps:
            ...
            if (
                ...
                continue
            if event.lab_result_strategy != strategyId:
                continue
            count += 1
        return count
    ...
    def runChoiceCoin(self):
        ...
        if (
            self.account.lab_account_choice_strategy is None
        ...
        choiceCoinCondition = json.loads(
            self.account.lab_account_choice_condition
        )
        ...
        while processed < self.total[self.longSym]:
            ...
            # key to seach in context
            contexKey = (
                f"indexData_{str(getDataStartTime)}_{str(getDataStopTime)}"
            )
            ...
            if (
                not isset(indexData, self.longSym)
                or not isset(indexData[self.longSym]["kline"], "1m")
                or len(indexData[self.longSym]["kline"]["1m"]) <= 0
            ):
                getDataStartTime = getDataStopTime
                getDataStopTime = (
                    getDataStartTime + self.blockDataLeng * 86400 * 1000
            ...
            longSymData = []
            for symbol in indexData:
                if symbol in self.totalDependSymbol:
                    continue
                for frame in indexData[symbol]:
                    if len(indexData[symbol][frame]) > len(longSymData):
                        longSymData = indexData[symbol][frame]
            ...
            for runtime in longSymData:
                ...
                if runtimeInt > choiceTime:
                    ...
                    for campId in self.campaignImps:
                        ...
                        choiceData = campImp.getChoiceData(
                            choiceTime - periodTime
                        )
                    ...
                    filterResult = self.filterChoiceCoin(
                        periodChoiceData, choiceTime, choiceCoinCondition
                    )
                    if not filterResult["result"]:
                        raise Exception(filterResult["message"])
                    choiceTime = ceil(runtimeInt / periodTime) * periodTime
        ...
        LabAccountWrapper().edit(
            ...
            {
                LabAccountWrapper.lab_account_choice_result: json.dumps(
                    self.choiceCoinData
                )
            },
        )
    ...
    def filterChoiceCoin(self, periodChoiceData, choiceTime, condition):
        ...
        for data in periodChoiceData:
            ...
            if not compareResult["result"]:
                return compareResult
            if compareResult["data"]:
                symbols.append(data["symbol"])

        if self.choiceCoinData is None:
            self.choiceCoinData = {}
