...
class CampaignImp(CampaignInterface):
    ...
    #choice results
    choiceData = {}
    active = True # incase check result return false, update active to false.
    ...
    def initial(self, optiParams, strategyId = None, rawRun=False):
        ...
        self.dataColumns = {}
        scanColumn(self.cfgStrategy, self.dataColumns)
    ...
    def setRun(self, checkData, runtime):
        ...
        self.frame1m = self.checkData[self.symbol]['kline']['1m'][0]
    ...
    def validateMonitorData(self):
        for symbol in self.needData:
            if(not isset(self.checkData, symbol)): 
                self.log("Does not enough data for mornitor:" + str(symbol))
                return False
            for frame in self.needData[symbol]:
                if(not isset(self.checkData[symbol]['kline'], frame)): 
                    self.log("Does not enough data for mornitor:" + str(symbol) + " " + str(frame))
                    return False
                if(-self.needData[symbol][frame] >= len(self.checkData[symbol]['kline'][frame])): 
                    self.log("Does not enough data for mornitor:" + str(symbol) + " " + str(frame) + " " + str(self.needData[symbol][frame]))
                    return False
        return True
    ...
    def validateOrderData(self):
        for symbol in self.needData:
            ...
            for frame in self.needData[symbol]:
                if(not isset(self.checkData[symbol]['kline'], frame)): 
                    self.log("Does not enough data for make order:" + str(symbol) + " " + str(frame))
                    return False
                if(-self.needData[symbol][frame] >= len(self.checkData[symbol]['kline'][frame])): 
                    self.log("Does not enough data for make order:" + str(symbol) + " " + str(frame) + " " + str(self.needData[symbol][frame]))
                    return False
                # if(Number(self.checkData[symbol]['kline'][frame][0]['lab_candle_' + frame + '_time']) < self.runtime): 
                #     self.log("Does not enough data for make order:" + str(symbol) + " " + str(frame) + " " + str(self.runtime))
                #     return False
        return True
    ...
    def monitorMatchedPart(self, flowName):
        ...
        if (phase < (self.getMatchLeng(flowName) - 1)) :
            ...
            #check enterpackage
            enterPackage = Number(get(afterCondition, 'enter_package', 100))
    ...
    def calculateElement(self, struct, dynamicIndex = None):
        ...
        if(structType == 'frame'):
            ...
            if(percent != 100):
                des = symbol + " Frame "+str(percent)+"% "+str(frame)+" "+str(column)+"("+str(-index)+")"
            else:
                des = symbol + " Frame "+str(frame)+" "+str(column)+"("+str(-index)+")"
            ...
            if(not isset(self.checkData, symbol)): return Reply.make(False, 'Can not get Data ' + symbol)
            if(not isset(self.checkData[symbol]['kline'], frame)): return Reply.make(False, 'Can not get Data ' + symbol + " " + frame)
            if(not isset(self.checkData[symbol]['kline'][frame], index)): return Reply.make(False, 'Can not get Data ' + symbol + " " + frame + " " + str(-index))
            ...
            try:
                ...
                return Reply.make(False, 'Can not get Data ' + symbol + " " + frame + " " + str(-index) + " " + str(column))
            ...
            if(isNumeric(value)): value = Number(value)
            if(percent != 100):
                des = "Event " + str(percent) + "% " + column
                if(percent != 100 and isNumeric(value)):
                    value = Number(value) * percent/100
            else:
                des = "Event " + column
            return Reply.make(True, des, value)
            ...
            if(logic == '+'): result = num1 + num2
            elif (logic == '-'): result = num1 - num2
            elif (logic == '*'): result = num1 * num2
            elif (logic == '/'): result = num1 / num2
            else: return Reply.make(False, 'Does not support logic ' + logic)
            return Reply.make(True, des, result)

        elif(structType == 'max' or structType == 'min'):
           
            frame = get(struct, 'frame', None)
            stopIndex = get(struct, 'stop_index', None)
            startIndex = get(struct, 'start_index', 0)
            column = get(struct, 'column', None)
            symbol = get(struct, 'symbol', self.symbol)
            percent = Number(get(struct, 'percent', 100))
           
            if(frame is None or stopIndex is None or column is None):
                return Reply.make(False, 'Please define symbol, frame, column and stop_index ' + json.dumps(struct))

            startIndex = Number(startIndex)
            if(startIndex > 0): return Reply.make(False, 'start_index must be lower than 0. ' + des)
            stopIndex = Number(stopIndex)
            if(stopIndex >= startIndex): return Reply.make(False, 'stop_index must be smaller than start_index. ' + des)
            
            if(percent != 100):
                des = f"{symbol} {percent}% {structType} {frame} {column} {startIndex} -> {stopIndex}"
            else:
                des = f"{symbol} {structType} {frame} {column} {startIndex} -> {stopIndex}"
            ...
            for i in range(startIndex, stopIndex-1, -1):
                ...
                calData['index'] = i
                calVal = self.calculateElement(calData)
                if(not calVal['result']): 
                    self.log(json.dumps(calVal), int(time()*1000))
                    self.log(json.dumps(self.checkData), int(time()*1000))
                    return calVal
                calVal = calVal['data']
                if(value is None):
                    value = calVal
        ...
        elif(structType == 'time'):
            frame = get(struct, 'frame', None)
            unit = get(struct, 'unit', 'minute')
            if(frame is None): return Reply.make(False, f"Please define frame for time")
            des = f"Order time {unit} in frame {frame}"
            frameTimeLeng = self.leapLeng[frame]
            modTime = (self.runtime % frameTimeLeng)/60000
            return Reply.make(True, des, modTime)

        elif(structType == 'BUSD'):
            indexCfg = get(struct, 'index', None)
            column = get(struct, 'column', None)
            symbol = get(struct, 'symbol', self.symbol)
            percent = Number(get(struct, 'percent', 100))

            index = -Number(indexCfg)
    ...
    def makeOrder(self, closePrice, phase, runtime):
        ...
        enterPackage = Number(get(condition, 'enter_package', 100))
    ...
    def createLeapCheckResult(self):
        ...
        print(needLeapData)
        #get index data
        needFrame = list(needLeapData[self.symbol].keys())
        indexData = {}
        for frame in needFrame:
            ...
            print(f"Get leap block data {self.symbol} " + frame + " from " + str(campStartTime) + " to " + str(campStopTime))
        ...
        for runtime in longSymData:
            ...
            for symbol in needLeapData:
                needDataSym = needLeapData[symbol]
                lastData = {}
