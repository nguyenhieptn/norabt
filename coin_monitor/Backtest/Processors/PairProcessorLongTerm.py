...
class PairProcessorLongTerm():

    
    def __init__(self, indicators = None, maxLeng = None) -> None:
        
        
        
        if(indicators is None):
            lengths = [30, 60, 120, 180, 360, 480]
            self.indicators = []
            for length in lengths:
                self.indicators += [
                    {"indicator":"sma", "source":"NET_F1B1_F2A1_MEAN", "params": length},
                    {"indicator":"stdev", "source":"NET_F1B1_F2A1_MEAN", "params": length},
                    {"indicator":"upper_band", "params": {'mid': f'sma_{length}_NET_F1B1_F2A1_MEAN', 'stdev': f'stdev_{length}_NET_F1B1_F2A1_MEAN'}, 'multi':3, 'name':f't{length}_upper_band_long'},
                    {"indicator":"lower_band", "params": {'mid': f'sma_{length}_NET_F1B1_F2A1_MEAN', 'stdev': f'stdev_{length}_NET_F1B1_F2A1_MEAN'}, 'multi':3, 'name':f't{length}_lower_band_long'},
                    
                    {"indicator":"sma", "source":"NET_F1A1_F2B1_MEAN", "params": length},
                    {"indicator":"stdev", "source":"NET_F1A1_F2B1_MEAN", "params": length},
                    {"indicator":"upper_band", "params": {'mid': f'sma_{length}_NET_F1A1_F2B1_MEAN', 'stdev': f'stdev_{length}_NET_F1A1_F2B1_MEAN'}, 'multi':3, 'name':f'n{length}_upper_band_long'},
                    {"indicator":"lower_band", "params": {'mid': f'sma_{length}_NET_F1A1_F2B1_MEAN', 'stdev': f'stdev_{length}_NET_F1A1_F2B1_MEAN'}, 'multi':3, 'name':f'n{length}_lower_band_long'},
                ]
            self.maxLeng = max(lengths)*60*2*4
        else:
            self.indicators = indicators
            self.maxLeng = maxLeng
    ...
    def processBlockData(self, datas):
        ...
        pairDataDF['timestamp_minute'] = pairDataDF['timestamp']//60000 * 60000
        pairDataDFMinute = pairDataDF.groupby('timestamp_minute').agg({
            'NET_F1B1_F2A1':'mean',
            'NET_F1A1_F2B1':'mean',
        }).rename(columns={
            'NET_F1B1_F2A1':'NET_F1B1_F2A1_MEAN',
            'NET_F1A1_F2B1':'NET_F1A1_F2B1_MEAN'
        })
        pairDataDFMinute['NET_F1B1_F2A1_MEAN'] = pairDataDFMinute['NET_F1B1_F2A1_MEAN'].shift(1)
        pairDataDFMinute['NET_F1A1_F2B1_MEAN'] = pairDataDFMinute['NET_F1A1_F2B1_MEAN'].shift(1)
