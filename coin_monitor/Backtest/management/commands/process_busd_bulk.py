...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        filter={"name": {"$regex": r"(\d{4}_\d{2}_\d{2})_([^_]+)_agg_trades"}}))
        collections.sort()
        collectionBySymbol = {}
        for collection in collections:

            rg = r"(?P<date>\d{4}_\d{2}_\d{2})_(?P<symbol>[^_]+)_agg_trades"
        ...
        print("Process BUSD all symbols were done.")  
    ...
    def openProc(self, symbol, optionDay,optionReset):
        print(f"Process busd {symbol}")
        pro = subprocess.Popen(f'python /home/ubuntu/linhvhv/coin_monitor/manage.py process_busd -s{symbol} {optionDay} {optionReset} > /home/ubuntu/linhvhv/coin_monitor/logs/process_busd_{symbol}.log', shell=True)
        pro.wait()        
        print(f"Process busd {symbol} done")
