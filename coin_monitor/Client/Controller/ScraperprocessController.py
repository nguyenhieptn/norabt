...
def add(request):
    ...
    instance = ScraperProcessWrapper().new(data) #type: ScraperProcess
    instance.save()
    return JsonResponse(Reply.make(True, 'Successed', ScraperProcessWrapper().toDict(instance)),json_dumps_params={'default':str})
...
def adds(request):
    ...
    instances = []
    for addData in data:
        instances.append(ScraperProcessWrapper().new(addData))
    ScraperProcessWrapper().bulk_create(instances)
...
def update(request):
    ...
    result = ScraperProcessWrapper().edit(key, value)
...
def read(request):
    ...
    model = ScraperProcessWrapper().all()
