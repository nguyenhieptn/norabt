...
from models.Wrappers.nora_monitor_wrapper import ScraperSystemWrapper
from models.nora_monitor import ScraperSystem
...
def add(request):
    ...
    instance = ScraperSystemWrapper().new(data) #type: ScraperSystem
    instance.save()
    return JsonResponse(Reply.make(True, 'Successed', ScraperSystemWrapper().toDict(instance)),json_dumps_params={'default':str})
...
def adds(request):
    ...
    instances = []
    for addData in data:
        instances.append(ScraperSystemWrapper().new(addData))
    ScraperSystemWrapper().bulk_create(instances)
...
def update(request):
    ...
    result = ScraperSystemWrapper().edit(key, value)
...
def read(request):
    ...
    model = ScraperSystemWrapper().all()
