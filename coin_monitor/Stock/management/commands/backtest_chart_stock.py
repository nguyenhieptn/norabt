...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.fileName = f'stock_backtest_{self.account}_{self.symbol}_{self.day}.html'
        print(f"Generate Chart for account:{self.account} {self.symbol} {self.day}")
        
        databaseName = None
        if(databaseName is None or databaseName == ''): databaseName = 'backtest_data_stock'
    ...
    def createChart(self):
        ...
        for row, ch in enumerate(charts):
            ...
            elif(ch['type'] == 'MacdPrice'):
                self.drawMacdPrice1m(row + 1, *ch['params'])
    ...
    def drawPrice(self, row):
        ...
        if(len(positionData) > 0):
            ...
            positionx2 = pd.to_datetime(positionData[LabResultsWrapper.lab_result_close_time].to_list(), unit='ms', utc=True).tz_convert(VN_TZ)
            positionData['type'] = positionData[LabResultsWrapper.lab_result_type].map(lambda x: "B" if x==1 else "S")
            ...
            self.fig.add_trace(go.Scatter(
                x=positionx2, 
                y=positionData[LabResultsWrapper.lab_result_sell_price], 
                name="Position Close", 
                marker=dict(color="orange", size=8), 
                mode="markers+text",
                text=positionData['state'],
                textposition="top center",
                textfont=dict(
                        color="orange",
                    )
        ...
        if(len(orderData) > 0):
            ...
            orderData['type'] = orderData[LabOrderWrapper.lab_order_type].map(lambda x: "<b>B</b>" if x==1 else "<b>S</b>")
            self.fig.add_trace(go.Scatter(
                x=positionx, 
    ...
    def drawBusdMacd(self, row, busdLen, ma, timeFrame=5, col="BUSD", merge=True):
        busdData=self.getData('BUSD')
        if(len(busdData) == 0): return

        hisCol = f"{col}_{busdLen}_{timeFrame}_{ma}_MACD"
        macCol = f"{col}_{busdLen}_{timeFrame}_{ma}_MACD"
       
        if(merge):
            busdData['timestamp_macd'] = busdData[BusdModel.timestamp].map(lambda x: x//(timeFrame*60)*(timeFrame*60))
        ...
        self.fig.add_trace(go.Scattergl(
            x=timex,
            y=busdDataGroup['BU_1d_60'],
            name='BU_1d_60'.upper()
        ), row=row, col=1)
        self.fig.add_trace(go.Scattergl(
            x=timex,
            y=busdDataGroup['SD_1d_60'],
            name='SD_1d_60'.upper()
        ), row=row, col=1)
        self.fig.add_trace(go.Scattergl(
            x=timex,
            y=busdDataGroup['BU_1d_60_5'],
            name='BU_1d_60_5'.upper()
        ), row=row, col=1)
        self.fig.add_trace(go.Scattergl(
            x=timex,
            y=busdDataGroup['SD_1d_60_5'],
            name='SD_1d_60_5'.upper()
        ), row=row, col=1)
    ...
    def drawBusdMacd1m(self, row, timeFrame, window, ticker=5, col="BUSD", merge=True):
        ...
        hisCol = f"{col}_{timeFrame}_{window}_{ticker}_MACD"
    ...
    def drawMacdPrice1m(self, row, ticker=5):
        data = self.getData('detailcandle_1m')
        if(len(data) == 0): return

        data['timestamp_frame'] = data[BusdModel.timestamp].map(lambda x: x//(ticker*60)*(ticker*60))
        busdDataGroup = data.groupby('timestamp_frame').last()
        timex = pd.to_datetime(busdDataGroup.index, unit='s' , utc=True).tz_convert(VN_TZ)

        busdDataGroup[f'FAST'] = busdDataGroup[f'close'].ewm(span=12, adjust=False, min_periods=12).mean()
        busdDataGroup[f'SLOW'] = busdDataGroup[f'close'].ewm(span=26, adjust=False, min_periods=26).mean()
        busdDataGroup[f'MACD'] = busdDataGroup[f'FAST'] - busdDataGroup[f'SLOW']
        busdDataGroup[f'SMOOTH'] = busdDataGroup[f'MACD'].ewm(span=9, adjust=False, min_periods=9).mean()
        busdDataGroup[f'MACDh'] = busdDataGroup[f'MACD'] - busdDataGroup[f'SMOOTH']
    ...
    def drawMacdhPrice1m(self, row, ticker=5):
        data = self.getData('detailcandle_1m')
        if(len(data) == 0): return
